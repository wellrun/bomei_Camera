...
static T_VOID keypad_init_diode(T_f_H_KEYPAD_CALLBACK callback_func,const T_PLATFORM_KEYPAD_PARM *keypad_parm)
{
    ...
    for (i=0; i<keypad_gpio_num; i++)
    {
        ...
        gpio_set_pin_level(m_keypad_gpio[i], GPIO_LEVEL_HIGH);
    }
    ...
    gpio_register_int_callback(gpio_power_key, GPIO_LEVEL_LOW, GPIO_INTERRUPT_DISABLE, keypad_power_callback);   
    gpio_register_int_callback(gpio_keypad_intr, GPIO_LEVEL_HIGH, GPIO_INTERRUPT_DISABLE, keypad_intr_callback);   
}
...
//    gpio_wgpio_pol(gpio_keypad_intr, GPIO_LEVEL_HIGH);
//    
//    gpio_set_pin_dir(gpio_power_key, GPIO_DIR_INPUT);
//    gpio_wgpio_mask(gpio_power_key, AK_TRUE);
//    gpio_wgpio_pol(gpio_power_key, GPIO_LEVEL_LOW);
//#elif (KEYPAD_MODE == 2)
//    T_U32 i;
//
...
//        gpio_set_pin_level(m_ucColumnGpio[i], GPIO_LEVEL_HIGH);
//    }
//
...
//        gpio_wgpio_pol(m_ucRowGpio[i], GPIO_LEVEL_HIGH);
//    }
//#endif
//}
...
static T_VOID keypad_power_callback(T_U32 pin, T_U8 polarity)
{
    ...
    if (polarity == GPIO_LEVEL_LOW)
    {
        ...
        gpio_set_int_p(gpio_power_key, GPIO_LEVEL_HIGH);
    }
    else
    {
        if (m_keypad_power_shake_timer != ERROR_TIMER)
        ...
        gpio_set_int_p(gpio_power_key, GPIO_LEVEL_LOW);
    }
...
static T_VOID keypad_power_shake_timer(T_TIMER timer_id, T_U32 delay)
{
    ...
    if (gpio_get_pin_level(gpio_power_key) == GPIO_LEVEL_LOW)
    {
        if (m_keypad_power_timer != ERROR_TIMER)
        {
            timer_stop(m_keypad_power_timer);
...
static T_VOID keypad_intr_callback(T_U32 pin, T_U8 polarity)
{
    ...
    if (polarity == GPIO_LEVEL_HIGH)
    {
        ...
        gpio_set_int_p(gpio_keypad_intr, GPIO_LEVEL_LOW);
    }
    else
    {
        if (m_keypad_intr_shake_timer != ERROR_TIMER)
        ...
        gpio_set_int_p(gpio_keypad_intr, GPIO_LEVEL_HIGH);
    }
...
static T_VOID keypad_intr_shake_timer(T_TIMER timer_id, T_U32 delay)
{
    ...
    if (gpio_get_pin_level(gpio_keypad_intr) == GPIO_LEVEL_HIGH)
    {
        if (m_keypad_intr_timer != ERROR_TIMER)
        {
            timer_stop(m_keypad_intr_timer);
...
static T_S32 keypad_scan_diode(T_VOID)
{
    ...
    for (i=0; i<keypad_gpio_num; i++)
    {
        ...
        for (k=0; k<keypad_gpio_num; k++)
        {
            ...
            if (gpio_get_pin_level(m_keypad_gpio[k]) == GPIO_LEVEL_HIGH)
            {
                for (j=0; j<keypad_gpio_num; j++)
                {
                    gpio_set_pin_dir(m_keypad_gpio[j], GPIO_DIR_OUTPUT);
                    gpio_set_pull_up_r(m_keypad_gpio[j], AK_FALSE);
                    gpio_set_pin_level(m_keypad_gpio[j], GPIO_LEVEL_HIGH);
                }
...
static T_BOOL keypad_intr_scan(T_BOOL isSHAKE)
{
    ...
    if ((savecount == 0) && (gpio_get_pin_level(gpio_keypad_intr) == GPIO_LEVEL_HIGH))
    {
