...
//NOTE:this function should not be removed!!!
...
static void cfltcvt( double value, char * buffer, char fmt, int precision )
{
    ...
    if ( fmt == 'g' )
    {
        ndigits = ecvtbuf( value, precision, & decpt, & sign, cvtbuf );
        magnitude = decpt - 1;
    ...
    if ( fmt == 'e' )
    {
        ndigits = ecvtbuf( value, precision + 1, & decpt, & sign, cvtbuf );

        if ( sign ) * buffer ++ = '-';
        * buffer ++ = * ndigits;
        if ( precision > 0 ) * buffer ++ = '.';
        memcpy( buffer, ndigits + 1, precision );
        buffer += precision;
        * buffer ++ = capnexp ? 'E' : 'e';
    ...
    {
        ndigits = fcvtbuf( value, precision, & decpt, & sign, cvtbuf );
        if ( sign ) * buffer ++ = '-';
        if (* ndigits )
        {
            if ( decpt <= 0 )
