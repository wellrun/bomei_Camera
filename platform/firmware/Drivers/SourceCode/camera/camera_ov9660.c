...
#define CAMERA_OV9660_ID        0x9660
...
#endif
#define OV9660_CAMERA_MCLK  24

static T_CAMERA_TYPE camera_ov9660_type = CAMERA_1P3M;
static T_NIGHT_MODE night_mode = CAMERA_DAY_MODE;
static T_CAMERA_MODE s_ov9660_CurMode = CAMERA_MODE_VGA;
...
static T_VOID cam_ov9660_open(T_VOID)
{
...
static T_BOOL cam_ov9660_close(T_VOID)
{	
	gpio_set_pin_level(GPIO_CAMERA_CHIP_ENABLE, !CAM_EN_LEVEL);
	gpio_set_pin_level(GPIO_CAMERA_AVDD, !gpio_pin_get_ActiveLevel(GPIO_CAMERA_AVDD));    
...
static T_U32 cam_ov9660_read_id(T_VOID)
{	
	T_U8 value = 0x00;
	T_U32 id = 0;
 ...
 */
static T_BOOL cam_ov9660_init(T_VOID)
{
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_mode(T_CAMERA_MODE mode)
{
    s_ov9660_CurMode = mode;
	switch(mode)
	{
		...
		case CAMERA_MODE_REC:                              //record mode
			...
			break;
		default:
            s_ov9660_CurMode = CAMERA_MODE_VGA;
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_exposure(T_CAMERA_EXPOSURE exposure)
{
	switch(exposure)
	{
		case EXPOSURE_WHOLE:
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_brightness(T_CAMERA_BRIGHTNESS brightness)
{
	switch(brightness)
	{
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_contrast(T_CAMERA_CONTRAST contrast)
{
	switch(contrast)
	{
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_saturation(T_CAMERA_SATURATION saturation)
{
	switch(saturation)
	{
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_sharpness(T_CAMERA_SHARPNESS sharpness)
{
	switch(sharpness)
	{
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_AWB(T_CAMERA_AWB awb)
{
	switch(awb)
	{
		case AWB_AUTO:
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_mirror(T_CAMERA_MIRROR mirror)
{
	switch(mirror)
	{
 ...
 * @retval
 */
static T_VOID cam_ov9660_set_effect(T_CAMERA_EFFECT effect)
{
	switch(effect)
	{
 ...
 */
static T_S32 cam_ov9660_set_digital_zoom(T_U32 width, T_U32 height)
{	
	...
	{			
		...
		case CAMERA_MODE_VGA:
			hrefstart = 100 + (1280 - 1280*width/640) / 4;
			hrefstop = 742 - (1280 - 1280*width/640) / 4;

			vrefstart = 7 + (1024 - 1024*height/480) / 4;
			vrefstop = 523 - (1024 - 1024*height/480) / 4;

			Camera_window_table[13] = 1280*width/640 / 8;
			Camera_window_table[15] = 1024*height/480 / 8;
			Camera_window_table[17] = width / 8;
			Camera_window_table[19] = height / 8;
			break;
		...
		case CAMERA_MODE_QVGA:
			hrefstart = 100 + (1280 - 1280*width/320) / 4;
			hrefstop = 742 - (1280 - 1280*width/320) / 4;

			vrefstart = 7 + (1024 - 1024*height/240) / 4;
			vrefstop = 523 - (1024 - 1024*height/240) / 4;

			Camera_window_table[13] = 1280*width/320 / 8;
			Camera_window_table[15] = 1024*height/240 / 8;
			Camera_window_table[17] = width / 8;
			Camera_window_table[19] = height / 8;
			break;
		...
		case CAMERA_MODE_PREV:
			hrefstart = 108 + (640 - width) / 2;
			hrefstop = 750 - (640 - width) / 2;
		...
		case CAMERA_MODE_REC:
			if ((width <= 352) && (height <= 288))    //VGA to CIF windowing
			{
				hrefstart = 108 + (640 - width) / 2;
				hrefstop = 750 - (640 - width) / 2;
			...
			{
				akprintf(C1, M_DRVSYS, "width = %d, heght = %d, over CIF mode\n", width, height);
				return 0;
			}
			break;
...
static T_VOID cam_ov9660_set_night_mode(T_NIGHT_MODE mode)
{
	switch(mode)
	{
...
static T_BOOL cam_ov9660_set_to_cap(T_U32 width, T_U32 height)
{
	float Mclk;
    ...
    {
        akprintf(C1, M_DRVSYS, "ov9660 unsupport %d & %d mode!\n", width, height);
        return AK_FALSE;
    }
	if ((CAMERA_MODE_VGA == Cammode)
		&& (width != 640) && (height != 480))
	{
		s_ov9660_CurMode = CAMERA_MODE_PREV;
		cam_ov9660_set_digital_zoom(width, height);
		mini_delay(300);
	}
	else
	{
...
static T_BOOL cam_ov9660_set_to_prev(T_U32 width, T_U32 height)
{
...
static T_BOOL cam_ov9660_set_to_record(T_U32 width, T_U32 height)
{	
...
static T_CAMERA_TYPE cam_ov9660_get_type(T_VOID)
{
	return camera_ov9660_type;
}

static T_CAMERA_FUNCTION_HANDLER ov9660_function_handler = 
{
	OV9660_CAMERA_MCLK,
	cam_ov9660_open,
	cam_ov9660_close,
	...
	cam_ov9660_init,
	cam_ov9660_set_mode,
	cam_ov9660_set_exposure,
	cam_ov9660_set_brightness,
	cam_ov9660_set_contrast,
	cam_ov9660_set_saturation,
	cam_ov9660_set_sharpness,
	cam_ov9660_set_AWB,
	cam_ov9660_set_mirror,
	cam_ov9660_set_effect,
	cam_ov9660_set_digital_zoom,
	cam_ov9660_set_night_mode,
	cam_ov9660_set_to_cap,
	cam_ov9660_set_to_prev,
	cam_ov9660_set_to_record,
	cam_ov9660_get_type
};

static int camera_ov9660_reg(void)
{
	camera_reg_dev(CAMERA_OV9660_ID, &ov9660_function_handler);
	return 0;
}
