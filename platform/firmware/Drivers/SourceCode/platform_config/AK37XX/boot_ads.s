...
#include "boot.h"
	...
	IMPORT |Image$$ER_ZI$$ZI$$Limit|  ;; MUST bss$$ZI, not bss
        IMPORT |Image$$inner_backup$$Base|
        IMPORT |Image$$inner_backup$$Limit|
...
nandflash_information
     ...
     space   32
#else
		EXPORT spiflash_param
     DCB      "SPIP"
spiflash_param
     DCD       0x5A5A5A5A      ;id
     DCD       0xFFFFFFFF      ;total size
     DCD       256             ;page size
     DCD       256             ;program size
     DCD       0x10000         ;erase size
     DCD       0x0             ;clock
     DCB       0x1             ;flag
     DCB       0x1c            ;protect mask
     DCB       0x0             ;resv1
     DCB       0x0             ;resv2
     space       32            ;name
#endif
...
;***************************************************************
    ...
    ;Copy inner code to inner_backup area
    ldr r0, =|Image$$inner_backup$$Limit|
    ldr r1, =|Image$$inner_backup$$Base|
    mov r2, #0
loop
    ldr r3, [r0, r2, lsl #2]
    str r3, [r1, r2, lsl #2]
