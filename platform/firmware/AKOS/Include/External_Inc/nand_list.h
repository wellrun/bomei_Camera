...
typedef struct Nand_phy_info{
    ...
    T_U8   custom_nd;//nand type flag, used to detect the original invilid block
        ...
        //NAND_TYPE_TOSHIBA_EXT:    0x4 小页SLC(),              大页SLC(),                      MLC([0,127/191/255], [0,2048/4096/8192])
    ...
    //bit29表示是否前后die，1表示有前后die
    //bit28表示是否奇偶plane，1表示有奇偶plane
    //注意: 如果(bit29和bit28)为'11'，则表示该chip包括4个plane。既有前后die，而一个die里面有奇偶plane
    
    //以下bit是为了支持24 bit ecc nand新特性的控制bit:
    //bit17表示是否支持randomizer，1表示支持
    //bit16表示是否支持single-plane copyback，1表示支持
    //bit15表示是否支持multi-plane copyback，1表示支持
    //bit14表示是否支持multi-plane read，1表示支持
    //bit13表示是否支持multi-plane erase，1表示支持
    //bit12表示是否支持multi-plane program，1表示支持
    
    //以下bit是为了解决page和block地址不连续而增加的控制bit:
    //bit11表示block number per die是否需要向上规整，1表示需要。如Toshiba TH58NVG6D2ETA20是2048 block/die(实际有2084 block/die)
         //为了对齐下一个die的block则需要规整为4096 block/die给底层驱动
    //bit10表示page number是否需要向上规整，1表示需要。如TLC是192page/block，为了对齐下一个block则需要规整为256page/block给驱动
