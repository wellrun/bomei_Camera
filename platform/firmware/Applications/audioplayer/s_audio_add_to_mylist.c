...
static T_VOID Audio_AddToList(T_pWSTR pFilePath);
#endif


void initaudio_add_to_mylist(void)
{
#ifdef SUPPORT_AUDIOPLAYER
    ...
    IconExplorer_SetTitleText(&pAudio_AddTo_MyList_Parm->IconExplorer, Res_GetStringByID(eRES_STR_AUDIOPLAYER_ADDTO_MYLIST), ICONEXPLORER_TITLE_TEXTCOLOR);
    //FileList_ToIconExplorer(&FileList, &pAudio_AddTo_MyList_Parm->IconExplorer);
...
void exitaudio_add_to_mylist(void)
{
#ifdef SUPPORT_AUDIOPLAYER
...
void paintaudio_add_to_mylist(void)
{
#ifdef SUPPORT_AUDIOPLAYER
...
unsigned char handleaudio_add_to_mylist(T_EVT_CODE event, T_EVT_PARAM *pEventParm)
{
#ifdef SUPPORT_AUDIOPLAYER
...
static T_VOID Audio_AddToList(T_pWSTR pFilePath)
{
    ...
    else
    {
        ...
        {
            ret = FileList_Add(&FileList, pFilePath, FILELIST_NO_SEARCH_SUB_NO_RECODE_FOLDER);
            if (FILELIST_ADD_SUCCESS == ret)
            {
                ret_sav = FileList_SaveFileList(&FileList, pListFilePath);
            }
			else if(FILELIST_ADD_NOSPACE == ret && FileList.ItemQty > FileList.ItemQtyMax)
			{
				ret_sav = AK_TRUE;
			}
		}
