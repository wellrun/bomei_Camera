...
static T_AUDIO_PITCH_PARM *pAudio_ToneParm;
extern T_AUDIOPLAYER *p_audio_player;

#endif

/*---------------------- BEGIN OF STATE s_audio_list_pitch_mode ------------------------*/
void initaudio_list_pitch_mode(void)
{
#ifdef SUPPORT_AUDIOPLAYER

    pAudio_ToneParm = (T_AUDIO_PITCH_PARM *)Fwl_Malloc(sizeof(T_AUDIO_PITCH_PARM));
    AK_ASSERT_PTR_VOID(pAudio_ToneParm, "initmp3_list_repeat_mode(): malloc error");

    MenuStructInit(&pAudio_ToneParm->IconExplorer);
    GetMenuStructContent(&pAudio_ToneParm->IconExplorer, mnMP3_PITCH_MODE);
    IconExplorer_SetFocus(&pAudio_ToneParm->IconExplorer, gs.AudioPitchMode);
    TopBar_DisableMenuButton();
#endif
}

void exitaudio_list_pitch_mode(void)
{
#ifdef SUPPORT_AUDIOPLAYER
...
void paintaudio_list_pitch_mode(void)
{
#ifdef SUPPORT_AUDIOPLAYER
...
unsigned char handleaudio_list_pitch_mode(T_EVT_CODE event, T_EVT_PARAM *pEventParm)
{
    ...
    case eNext:
        ...
        if (focusID != gs.AudioPitchMode)
        {
            gs.AudioPitchMode = (T_U8)focusID;            
