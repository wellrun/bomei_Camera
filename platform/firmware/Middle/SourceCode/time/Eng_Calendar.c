...
T_S16 iLunarYear, iLunarMonth, iLunarDay;
...
T_S16 WeekDay(T_S16 iYear, T_S16 iMonth, T_S16 iDay)
{
    ...
    T_S16 monthday[]={0,3,3,6,1,4,6,2,5,0,3,5};
    T_S16 iDays = (iYear-1)%7 + (iYear-1)/4 - (iYear-1)/100 +(iYear-1)/400;
...
{
    T_S16 height =0 ,low =29, iBit;
    if(iLunarYear < START_YEAR) 
        return 30L;

    iBit = 16 - iLunarMon;

    if(iLunarMon > GetLeapMonth(iLunarYear) && GetLeapMonth(iLunarYear))
           iBit --;

    if(gLunarMonthDay[iLunarYear - START_YEAR] & (1<<iBit))
            low ++;
        
    if(iLunarMon == GetLeapMonth(iLunarYear))
    {
...
T_S16 LunarYearDays(T_S16 iLunarYear)
{
    ...
    */
    ...
    for(i=1; i<=12; i++)
    { 
        T_S32  tmp = LunarMonthDays(iLunarYear ,i); 
        days += (tmp>>16);
        days += (tmp&0xFFFF);
    }
    return (T_S16)days;
...
{
    T_S16 flag = gLunarMonth[(iLunarYear - START_YEAR)/2];
...
T_S32 CalcDateDiff(T_S16 iEndYear, T_S16 iEndMonth, T_S16 iEndDay,
                            T_S16  iStartYear, T_S16 iStartMonth, T_S16 iStartDay)
{
    T_S16 monthday[]={0, 31, 59 ,90, 120, 151, 181, 212, 243, 273, 304, 334}; 

    //计算两个年份1月1日之间相差的天数
    T_S32 iDiffDays =(iEndYear - iStartYear)*365;
...
void  l_CalcLunarDate(T_S32 iSpanDays)
{
       ...
       if(iSpanDays <49)
    {
        ...
        { 
          iMonth = 11;  
          iDay   = (T_S16)(11+iSpanDays);
        }
        else
        {
            iMonth = 12;
            iDay   =  (T_S16)(iSpanDays -18);
        }
        return ;
    }
    //下面从阴历1901年正月初一算起
...
T_S16 GetLunarDate(T_S16 iYear, T_S16 iMonth, T_S16 iDay)                //计算阴历日期
{
...
void FormatMonth(T_S16 iMonth, T_U16 *pBuffer, T_BOOL bLunar)
{
   if(!bLunar && iMonth==1)
   {
       Utl_UStrCpy(pBuffer, Res_GetStringByID(eRES_STR_CAL_LUNAR_1M));
       return;
   }

   if(iMonth<=10)
   {
       memcpy(pBuffer, Res_GetStringByID(eRES_STR_CAL_LUNAR_MONTH) + (iMonth -1), 2);
       Utl_UStrCpy(pBuffer+1 , Res_GetStringByID(eRES_STR_CAL_LUNAR_M));
       return;
   }

   if (iMonth == 11)
       Utl_UStrCpy(pBuffer, Res_GetStringByID(eRES_STR_CAL_LUNAR_11M));
   else
       Utl_UStrCpy(pBuffer, Res_GetStringByID(eRES_STR_CAL_LUNAR_12M));
   
    Utl_UStrCpy(pBuffer+2 , Res_GetStringByID(eRES_STR_CAL_LUNAR_M));
...
void FormatLunarDay(T_S16  iDay, T_U16 *pBuffer)
{
    if(iDay != 20 && iDay !=30)
    {
        memcpy(pBuffer, Res_GetStringByID(eRES_STR_CAL_LUNAR_CHU) + (iDay-1)/10 ,2);
        memcpy(pBuffer+1, Res_GetStringByID(eRES_STR_CAL_LUNAR_DAY) + ((iDay-1)%10) ,2);
        pBuffer[2]='\0';
    }
    else
    {
        memcpy(pBuffer, Res_GetStringByID(eRES_STR_CAL_LUNAR_CHU) + iDay/10, 2);
        Utl_UStrCpy(pBuffer+1, Res_GetStringByID(eRES_STR_CAL_LUNAR_DAY) +9);
    }
}
