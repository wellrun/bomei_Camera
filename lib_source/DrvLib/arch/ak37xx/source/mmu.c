...
T_VOID MMU_Init(T_U32 mmutt_start_addr)
{
    ...
    {
        akprintf(C3, M_DRVSYS, "mmu start address 0x%x is not aligned to 16K!!\n", mmutt_start_addr);
        while(1);
    }
    ...
    MMU_SetMTT(0x20000000,0x20400000-1,0x20000000,RW_NCNB);  //SFR !!
    MMU_SetMTT(0x30000000,0x30000000+drv_get_ram_size()-1,0x30000000,RW_CB);   //DRAM
    MMU_SetMTT(0x48000000,0x48100000-1,0x48000000,RW_NCNB);  //L2
    MMU_SetMTT(0x70000000,0x70100000-1,0x70000000,RW_NCNB);  //USB
