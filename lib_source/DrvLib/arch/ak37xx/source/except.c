...
static T_fCExceptCallback m_excpt_cb=AK_NULL;
...
typedef  struct
{
        ...
        T_U32   old_pc;                          //thread current PC value
        T_U32   old_spsr;
        T_U32   old_reg[13];
        T_U32   old_sp;        
        T_U32   old_stack_value[20];
...
*/
T_VOID system_error_check( T_U32 *addr_sp )
{
        T_U32  *current_stack = AK_NULL;
        T_U32  *old_stack = AK_NULL;
        T_U32   temp = 0;
        T_U32   spsr_value = 0;
        T_U32   error_type = 0;
        int     i;
        ...
        //      old_stack = (T_U32 *)exception_context.old_sp;
        for     (i = 0; i < 20; i++)
        {
...
/**
 @brief:
    ...
    ---------------------------------------------
 */
T_VOID swi_dispatch(T_U32 id, T_U32 value, T_U32 number)
{
    /* ARM Semihosting SWI */
    if (number == 0x123456)
    {   
