...
volatile  T_BOOL  g_bUvcSending = AK_FALSE;

static T_pUVC_DEV s_pUvcDev = AK_NULL;
static T_U8 s_ConfigData[500] = {0};

static T_VOID uvc_dev_msg_ctrl_proc(T_U32* pMsg,T_U32 len);
static T_VOID uvc_dev_msg_send_proc(T_U32* pMsg,T_U32 len);
static T_VOID vc_ctrl_handle(T_U8 unit, T_U8 cs , T_U32 cur_val1,T_U32 cur_val2);
static T_VOID vs_ctrl_handle(T_U8 unit, T_U8 cs , T_U32 cur_val1,T_U32 cur_val2);
static T_BOOL uvc_enable();
static T_BOOL uvc_disable();
static T_VOID uvc_reset(T_U32 mode);
static T_VOID uvc_suspend();
static T_VOID uvc_resume();
static T_VOID uvc_configok();
static T_VOID uvc_send_finish();
static T_BOOL uvc_class_callback(T_CONTROL_TRANS *pTrans);
static T_U8 *uvc_get_dev_qualifier_desc(T_U32 *count);
static T_U8 *uvc_get_dev_desc(T_U32 *count);
static T_U8 *uvc_get_cfg_desc(T_U32 *count);
static T_U8 *uvc_get_other_speed_cfg_desc(T_U32 *count);
static T_U8 *uvc_get_str_desc(T_U8 index, T_U32 *count);
...
const static T_UVC_VIDEOCONTROL_INTERFACE_HEADER_DESCRIPTOR s_UvcVideoCtrlInterfaceHeaderDesc=
{
    ...
    0x005B8D80,                                                 ///The device clock frequency is 6000000 Hz
...
const static T_USB_ENDPOINT_DESCRIPTOR  s_UvcEp1Desc =
{
    ...
    (ENDPOINT_DIR_IN + EP4_INDEX),        ///<  IN endpoint 1
...
const static T_UVC_VIDEOSTREAM_FRAME_DESCRIPTOR s_UvcMjpegFrame[UVC_FRAME_NUM]=
{
    ...
    //frame3:176x144
    {
        ...
        176,                                               ///<width
        144,                                               ///<height
        176*144*36,                                        ///<min bit rate
        176*144*36,                                        ///<max bit rate
        176*144*3/2,                                       ///<max frame buffer size
        0x00051615,                                        ///<Default frame interval is 333333x100ns (30fps).
        0x00,                                              ///<  Continuous frame interval
        0x00051615,                                        ///<  Minimum frame interval is 333333x100ns (30fps)
        0x00051615,                                        ///<  Maximum frame interval is 333333x100ns(30fps).
...
const static T_UVC_VIDEOSTREAM_FRAME_DESCRIPTOR s_UvcUncompressedFrame[UVC_FRAME_NUM]=
{
    ...
    //frame3:176x144
    {
        ...
        176,                                               ///<width
        144,                                               ///<height
        176*144*30*16,                                     ///<min bit rate
        176*144*30*16,                                     ///<max bit rate
        176*144*2,                                         ///<max frame buffer size
        0x00051615,                                        ///<Default frame interval is 333333x100ns (30fps).
        0x00,                                              ///<  Continuous frame interval
        0x00051615,                                        ///<  Minimum frame interval is 333333x100ns (30fps)
        0x00051615,                                        ///<  Maximum frame interval is 333333x100ns(30fps).
...
/* language descriptor */
const static T_U8 s_UvcString0[] = 
{
...
/*string descriptor*/
const static T_U8 s_UvcString1[] =
{
    12,             
    0x03,               
    'A',0,          
...
const static T_U8 s_UvcString2[] = 
{
    12,             
    0x03,               
    'U',0,          
...
const static T_U8 s_UvcString3[] = 
{
    12,             
    0x03,               
    '1',0,          
...
T_VIDEO_PROBE_COMMIT_CONTROLS s_VideoProbeCommitControls =
{
    ...
    0x00051615,         //  Frame interval is 30fps
    0x0000,
    0x0000,
    0x0000,
    0x0000,
...
static T_U32 s_UvcCtrlMap[UVC_SUPPORTED_CTRL_NUM]=
{
...
{
    if (AK_NULL == pFrameRes || FrameId > UVC_FRAME_NUM || FrameId < 1)
    {
...
*/
T_BOOL uvc_set_ctrl(T_eUVC_CONTROL dwControl, T_U32 ulMin, T_U32 ulMax, T_U32 ulDef,T_U32 ulRes)
{
...
T_BOOL uvc_init_desc()
{
    ...
    memcpy(s_ConfigData,(T_U8*)&s_UvcConfigDesc,sizeof(T_USB_CONFIGURATION_DESCRIPTOR));
    cnt += sizeof(T_USB_CONFIGURATION_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcInterfaceAssociationDesc,sizeof(T_UVC_INTERFACE_ASSOCIATION_DESCRIPTOR));
    cnt += sizeof(T_UVC_INTERFACE_ASSOCIATION_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcVciDesc,sizeof(T_USB_INTERFACE_DESCRIPTOR));
    cnt += sizeof(T_USB_INTERFACE_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcVideoCtrlInterfaceHeaderDesc,sizeof(T_UVC_VIDEOCONTROL_INTERFACE_HEADER_DESCRIPTOR));
    cnt += sizeof(T_UVC_VIDEOCONTROL_INTERFACE_HEADER_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcCameraTerminalDesc,sizeof(T_UVC_CAMERA_TERMINAL_DESCRIPTOR));
    cnt += sizeof(T_UVC_CAMERA_TERMINAL_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcInputTerminalDesc,sizeof(T_UVC_INPUT_TERMINAL_DESCRIPTOR));
    cnt += sizeof(T_UVC_INPUT_TERMINAL_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcOutputTerminalDesc,sizeof(T_UVC_OUTPUT_TERMINAL_DESCRIPTOR));
    cnt += sizeof(T_UVC_OUTPUT_TERMINAL_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcSelectUnitDesc,sizeof(T_UVC_SELECTOR_UNIT_DESCRIPTOR));
    cnt += sizeof(T_UVC_SELECTOR_UNIT_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcProcessUnitDesc,sizeof(T_UVC_PROCESSING_UNIT_DESCRIPTOR));
    cnt += sizeof(T_UVC_PROCESSING_UNIT_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcEp1Desc,sizeof(T_USB_ENDPOINT_DESCRIPTOR));
    cnt += sizeof(T_USB_ENDPOINT_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcVideoCtrlIntEpDesc,sizeof(T_UVC_VIDEOCONTROL_INTERRUPT_ENDPOINT_DESCRIPTOR) );
    cnt += sizeof(T_UVC_VIDEOCONTROL_INTERRUPT_ENDPOINT_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcVsiDesc,sizeof(T_USB_INTERFACE_DESCRIPTOR));
    cnt += sizeof(T_USB_INTERFACE_DESCRIPTOR);
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcVideoStreamInterfaceInputHeaderDesc, sizeof(T_UVC_VIDEOSTREAM_INTERFACE_INPUT_HEADER_DESCRIPTOR));
    cnt +=  sizeof(T_UVC_VIDEOSTREAM_INTERFACE_INPUT_HEADER_DESCRIPTOR);
    if (UVC_STREAM_MJPEG == s_pUvcDev->ucFormat)
    {
        memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcMjpegFormatDesc, sizeof(T_UVC_MJPEG_VIDEO_FORMAT_DESCRIPTOR));
        cnt +=  sizeof(T_UVC_MJPEG_VIDEO_FORMAT_DESCRIPTOR);
        memcpy(s_ConfigData+cnt,(T_U8*)s_UvcMjpegFrame, sizeof(s_UvcMjpegFrame));
        cnt +=  sizeof(s_UvcMjpegFrame);
    }
    else
    {
        memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcUncompressedFormatDesc, sizeof(T_UVC_UNCOMPRESSED_VIDEO_FORMAT_DESCRIPTOR));
        cnt += sizeof(T_UVC_UNCOMPRESSED_VIDEO_FORMAT_DESCRIPTOR);
        memcpy(s_ConfigData+cnt,(T_U8*)s_UvcUncompressedFrame, sizeof(s_UvcUncompressedFrame));
        cnt +=  sizeof(s_UvcUncompressedFrame);
    }
    memcpy(s_ConfigData+cnt,(T_U8*)&s_UvcEp2Desc,sizeof(T_USB_ENDPOINT_DESCRIPTOR));
    cnt += sizeof(T_USB_ENDPOINT_DESCRIPTOR);
...
T_U32 uvc_payload(T_U8* pPayload, T_U8* pData, T_U32 dwSize)
{
    ...
    if (pPayload == AK_NULL || pData == AK_NULL)
    {
...
{
    g_bUvcSending = AK_FALSE;
    
    DrvModule_Terminate_Task(DRV_MODULE_UVC);
    uvc_disable();
    if (AK_NULL != s_pUvcDev)
    {
        drv_free(s_pUvcDev);
    }
...
T_U32 uvc_parse_yuv(T_U8 *pYUV,T_U8 *y,T_U8 *u,T_U8 *v,T_U32 width,T_U32 height,T_U8 yuv_format)
{
    ...
    if (pYUV == AK_NULL || y == AK_NULL || u == AK_NULL || v == AK_NULL)
    {
        return 0;
    }
    //yuv 4:2:2
...
static T_VOID uvc_dev_msg_ctrl_proc(T_U32* pMsg,T_U32 len)
{
    ...
    switch (message->ucMsgId)
    {
        ...
        case MSG_VS_CTRL:
            ...
            {
                if (AK_NULL != s_pUvcDev->fVsCtrlCallBack)
                {
                    g_bUvcSending = AK_TRUE;
...
static T_VOID uvc_dev_msg_send_proc(T_U32* pMsg,T_U32 len)
{
        if (AK_NULL != s_pUvcDev->fFrameSentCallBack)
        {
...
static T_BOOL uvc_enable()
{
    ...
    //usb_slave_set_rx_callback(EP1_INDEX, uvc_receive_notify, uvc_receive_finish);
...
static T_BOOL uvc_disable()
{
    ...
    usb_slave_set_tx_callback(EP2_INDEX, AK_NULL);
...
static T_VOID uvc_suspend()
{
    ...
    //clr stall status to avoid die when uvc task is waiting clr stall to send csw 
    usb_slave_set_ep_status(EP2_INDEX,0);
    usb_slave_set_ep_status(EP1_INDEX,0);
}
...
static T_VOID uvc_send_finish()
{
    DrvModule_Send_Message(DRV_MODULE_UVC, UVC_DEV_MSG_SEND, AK_NULL);
}
                        ...
                        case GET_CUR:
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    ...
                                    {
                                        case UVC_PU_ID :       
                                            memcpy(pTrans->buffer,(T_U8 *)(&s_pUvcDev->tCtrlSetting[ctrl_id].ulCur),s_pUvcDev->tCtrlSetting[ctrl_id].ulLen);
                                ...
                                case UVC_VSI_ID :
                                    ...
                                    {
                                        case VS_PROBE_CONTROL:
                                            memcpy(pTrans->buffer,(T_U8 *)(&s_VideoProbeCommitControls),sizeof(T_VIDEO_PROBE_COMMIT_CONTROLS));
                        ...
                        case GET_MIN:
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    ...
                                    {
                                        case UVC_PU_ID :       
                                            memcpy(pTrans->buffer,(T_U8 *)(&s_pUvcDev->tCtrlSetting[ctrl_id].ulMin),s_pUvcDev->tCtrlSetting[ctrl_id].ulLen);
                                            pTrans->data_len = s_pUvcDev->tCtrlSetting[ctrl_id].ulLen;
                                            break;
                        ...
                        case GET_MAX:
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    ...
                                    {
                                        case UVC_PU_ID :       
                                            memcpy(pTrans->buffer,(T_U8 *)(&s_pUvcDev->tCtrlSetting[ctrl_id].ulMax),s_pUvcDev->tCtrlSetting[ctrl_id].ulLen);
                                            pTrans->data_len = s_pUvcDev->tCtrlSetting[ctrl_id].ulLen;
                                            break;
                        ...
                        case GET_RES:
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    ...
                                    {
                                        case UVC_PU_ID :       
                                            memcpy(pTrans->buffer,(T_U8 *)(&s_pUvcDev->tCtrlSetting[ctrl_id].ulRes),s_pUvcDev->tCtrlSetting[ctrl_id].ulLen);
                                            pTrans->data_len = s_pUvcDev->tCtrlSetting[ctrl_id].ulLen;
                                            break;
                        ...
                        case GET_DEF:
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    ...
                                    {
                                        case UVC_PU_ID :       
                                            memcpy(pTrans->buffer,(T_U8 *)(&s_pUvcDev->tCtrlSetting[ctrl_id].ulDef),s_pUvcDev->tCtrlSetting[ctrl_id].ulLen);
                                            pTrans->data_len = s_pUvcDev->tCtrlSetting[ctrl_id].ulLen;
                                            break;
                                    ...
                                    break;
                                case UVC_VSI_ID :
                                    akprintf(C1, M_DRVSYS, "Unsupport GET_DEF: VSI\n");
                                    ret = AK_FALSE;
                                    break;
                ...
                if (pTrans->dev_req.bRequest == SET_CUR)
                {
                    ...
                    switch (LOBYTE(pTrans->dev_req.wIndex))
                    {
                        ...
                        case UVC_VCI_ID :
                            ...
                            msg.ulParam4 = *((T_U32 *)(pTrans->buffer+4));
                            DrvModule_Send_Message(DRV_MODULE_UVC, UVC_DEV_MSG_CTRL, (T_U32*)&msg);
                            break;
                        ...
                        case UVC_VSI_ID :
                            ...
                            switch (HIBYTE(pTrans->dev_req.wValue))
                            {
                                case VS_PROBE_CONTROL:
                                    ...
                                    memcpy((T_U8 *)&s_VideoProbeCommitControls,(T_U8 *)pTrans->buffer,4); 
...
static T_U8 *uvc_get_dev_qualifier_desc(T_U32 *count)
{
    *count = sizeof(s_UvcDeviceQualifierDesc);
    return (T_U8 *)&s_UvcDeviceQualifierDesc;
}
...
static T_U8 * uvc_get_other_speed_cfg_desc(T_U32 *count)
{
    ...
    memcpy(s_ConfigData, (T_U8 *)&s_UvcOtherSpeedConfigDesc, *(T_U8 *)&s_UvcOtherSpeedConfigDesc);
...
static T_U8 *uvc_get_str_desc(T_U8 index, T_U32 *count)
{
    if(index == 0)
    {
        *count = sizeof(s_UvcString0);
        return (T_U8 *)s_UvcString0;
    }
    else if(index == 1)
    {
        *count = sizeof(s_UvcString1);
        return (T_U8 *)s_UvcString1;
    }
    else if(index == 2)
    {
        *count = sizeof(s_UvcString2);
        return (T_U8 *)s_UvcString2;
    }
      else if(index == 3)
    {
        *count = sizeof(s_UvcString3);
        return (T_U8 *)s_UvcString3;
    }
    return AK_NULL;
}
