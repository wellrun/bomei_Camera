...
//only for linux kener
extern T_BOOL ak_chip_is_37L(void);
#if defined (CONFIG_ARCH_AK98)
#define FHA_CHIP_SET_TYPE      FHA_CHIP_980X
#elif defined (CONFIG_ARCH_AK37)
#define FHA_CHIP_SET_TYPE      (ak_chip_is_37L() ? FHA_CHIP_37XXL : FHA_CHIP_37XX)
#endif
 ...
 *                  note: e.g.  检测版本是xxx v2.1.03_xxx
 *                  输入的版本lib_info和早期烧录的版本进行检测
 *                  检测的规则: xxx v2.1.  需要检测的lib_info和库中的必须一样
 *                                               03             需要检测的lib_info必须大于或等于库
 *                                                               中的版本，如:库是04将失败，00-03都通过
 *                                               _xxx         将不再检测
...
/************************************************************************
 * NAME:     FHA_read_boot_begin
 * FUNCTION  set read boot start init para
 * PARAM:    [in] bin_param--Bin file info struct
 * RETURN:   success return FHA_SUCCESS, fail retuen FHA_ FAIL
**************************************************************************/
...
/************************************************************************
 * NAME:     FHA_read_boot
 * FUNCTION  read boot to buf from medium
 * PARAM:    [out]pData-----need to read bin data buf pointer addr
 *           [in] data_len--need to read bin data length
 * RETURN:   success return FHA_SUCCESS, fail retuen FHA_ FAIL
**************************************************************************/
...
/************************************************************************
 * NAME:     FHA_write_AllDatat_begin
 * FUNCTION  set write alldata into spiflash start init para
 * PARAM:    [in] bin_param--Bin file info struct
 * RETURN:   success return FHA_SUCCESS, fail retuen FHA_ FAIL
**************************************************************************/
