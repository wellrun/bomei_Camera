...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	//}
	all_mount_event = CreateEvent(NULL, TRUE, FALSE, NULL);
	m_budisk_burn = FALSE;
...
void CMainFrame::OnTbtnStart() 
{
	...
	ResetEvent(all_mount_event);
	theConfig.run_time = 0;
	m_budisk_burn = TRUE;
	...
	theConfig.bAuto_Burnflag = check;
	if ((g_workTotalnum = BT_Start()) != 0)
	{
        m_total_num += g_workTotalnum;
		theConfig.burn_totalnum += g_workTotalnum;
        g_workThreadnum = 0;
		...
		if(diskName[i] >= 'A' && diskName[i] <= 'Z')
		{
           ...
           frmLogfile.WriteLogFile(0,"BT_AttachUSB vol = %d diskName = %c m_udisk_state %d\n",vol, diskName[i], m_udisk_state[vol]);
			if(UDISK_NULL == m_udisk_state[vol])
			{
				...
				frmLogfile.WriteLogFile(0,"BT_AttachUSB2 %d, attach_flag:%d, auto_check:%d\n", index, attach_flag, check);
...
void CMainFrame::fail_in_module_burn(int id, LPCTSTR str)
{
    ...
    if (m_worknum != 0)
    {
        ...
        if (g_workTotalnum != 0)
            g_workTotalnum--;
		if (theConfig.burn_totalnum != 0)
            theConfig.burn_totalnum--;
...
LRESULT CMainFrame::On_Process(WPARAM wp, LPARAM lp)
{
	...
	UINT total_length;
	UINT total_length_new;
	UINT pos;
	UINT  i = 0;
	FLOAT parttion_length = 0;
	...
	if ((theConfig.planform_tpye == E_ROST_PLANFORM) && ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type)))
	{
		...
		else
		{
			...
			{
				total_length_new = download_length_new_mtd_totallen[port];
				pos = bin_pos + (int)(80*((double)(length-download_length_data)/total_length_new));
			}
		}
	}
	else
	{
		...
		total_length = download_length_data + download_length_mtd + download_length_udisk + (UINT)parttion_length;
		pos = (int)(100*((double)length/total_length));
	}
...
LRESULT CMainFrame::On_Message(WPARAM wp, LPARAM lp)
{
	...
	case MESSAGE_READ_OTP_SERIAL:
		pView->SetStatDesp(port, theApp.GetString(IDS_BURN_READ_OTP_SERIAL));
		break;
	case MESSAGE_READ_OTP_SERIAL_FAIL:
		pView->SetStatDesp(port, theApp.GetString(IDS_BURN_READ_OTP_SERIAL_FAIL));
		terminate_mission(port, theApp.GetString(IDS_BURN_READ_OTP_SERIAL_FAIL));
		break;
	case MESSAGE_WRITE_OTP_SERIAL:
		pView->SetStatDesp(port, theApp.GetString(IDS_BURN_WRITE_OTP_SERIAL));
		break;
	case MESSAGE_WRITE_OTP_SERIAL_FAIL:
		pView->SetStatDesp(port, theApp.GetString(IDS_BURN_WRITE_OTP_SERIAL_FAIL));
		terminate_mission(port, theApp.GetString(IDS_BURN_WRITE_OTP_SERIAL_FAIL));
		break;
	case MESSAGE_OTP_SERIAL_EXIST:
		pView->SetStatDesp(port, theApp.GetString(IDS_BURN_OTP_SERIAL_EXIST));
		break;
	...
	case MESSAGE_BURN_SERIAL_ADDR_SHOW:
		...
		break;
	case MESSAGE_BURN_OTP_SERIAL_SHOW:
		//烧录完成后显示当前otp序列号
		OnOtp_serial(port, TRUE); 
		break;
...
void CMainFrame::CountDownloadLength()
{
	...
	if ((theConfig.chip_type == CHIP_37XX || theConfig.chip_type == CHIP_37XX_L) && (theConfig.burn_mode == E_CONFIG_NAND))
	{
		hFile = CreateFile(theApp.ConvertAbsolutePath(theConfig.path_nandretry), GENERIC_READ , FILE_SHARE_READ , NULL , 
			OPEN_EXISTING , FILE_ATTRIBUTE_NORMAL , NULL);
		if(hFile != INVALID_HANDLE_VALUE)
		{
			dwSize = GetFileSize(hFile, NULL);
			...
			{
				str.Format(_T("Cannot find file: %s"), theConfig.path_nandretry);
				MessageBox(str);
...
void CMainFrame::OnSequenceAddr(int port, BOOL falg) 
{
	...
	CBurnToolView *pView = (CBurnToolView *)GetActiveView();
//	_tcsncpy(buf, theConfig.sequence_current, MAX_MAC_SEQU_ADDR_COUNT);
	if (port == 0)
	{
		_tcsncpy(buf, theConfig.g_sequence_show_current[port], MAX_MAC_SEQU_ADDR_COUNT);
...
void CMainFrame::OnOtp_serial(int port, BOOL falg) 
{	
	USES_CONVERSION;
	...
	pView->SetCurrentSequenceAddr(port, theConfig.g_OTP_serial_show_current[port]);
...
DWORD WINAPI CMainFrame::online_image_create_thread_snowdirdL(LPVOID para)
{
	...
	pMF->download_length_new_mtd_totallen[nID - 1] = 0;
	disk[0] = 'Z';
	DriverID = 0;
	...
	for(i = 0; i < g_disk_count; i++)
	{
		...
		if(hFile != INVALID_HANDLE_VALUE)
		{
			...
			if(dwSize != 0xFFFFFFFF)
			{
				...
				pMF->download_length_new_mtd_totallen[nID - 1] = pMF->download_length_new_mtd;
			}
...
void SetPartInfo(UINT nID, T_DRIVER_INFO *pDirverInfo, UINT num, UINT SectorPerPage, UINT SectorSize, UINT nDISKB_Enlarge)
{
    ...
    g_disk_count = theConfig.format_count;
    for(i = 0; i < g_disk_count; i++)
    {
