...
static UINT reg_addr_sdram_sun3C[] = 
{
	0x0800007c,
	0x2002d004,
	0x2002d000,
...
static UINT reg_value_sdram_sun3C[] = 
{
	0x008F0000,
	0x155889cb, //ac row,AHB=32,now 64MB rtl sim mode, old 0x155889cb 16MB SDRAM
	0xe0170000,
	0xe0120400,
	0xe0110000,
	0xe0110000,
...
UINT config_ram_param(T_RAM_REG *ram)
{
    ...
    if (CHIP_37XX == theConfig.chip_type || CHIP_37XX_L == theConfig.chip_type || CHIP_37XX_C == theConfig.chip_type)
    {
        if (0 == regCnt)
        {
	...
	/*
	else if (CHIP_37XX_C == theConfig.chip_type)
	{
		if (0 == regCnt)
        {
            regCnt = sizeof(reg_addr_sdram_sun3C) / sizeof(UINT);
			
            for(i=0; i<regCnt; i++)
            {
                ram[i].reg_addr = reg_addr_sdram_sun3C[i]; 
                ram[i].reg_value = reg_value_sdram_sun3C[i];
            }
        }
