                               ...
                               UINT nID, DWORD dwStyle)
{
    ...
    return Create(lpszWindowName, pParentWnd, nID, dwStyle);//Create
}
                               ...
                               DWORD dwStyle)
{
    ...
    CString wndclass = ::AfxRegisterWndClass(CS_DBLCLKS,
        ...
        ::GetSysColorBrush(COLOR_BTNFACE), 0);//GetSysColorBrush
...
void CSizingControlBar::EnableDocking(DWORD dwDockStyle)
{
    ...
    if (m_pDockContext == NULL)
        m_pDockContext = new CSCBDockContext(this);//CSCBDockContext

    // permanently wire the bar's owner to its current parent
    if (m_hWndOwner == NULL)
        m_hWndOwner = ::GetParent(m_hWnd);//GetParent
}
#endif
...
{
    if (baseCSizingControlBar::OnCreate(lpCreateStruct) == -1)//OnCreate
        return -1;
...
LRESULT CSizingControlBar::OnSetText(WPARAM wParam, LPARAM lParam)
{
    ...
    if (IsFloating() &&
        GetParentFrame()->IsKindOf(RUNTIME_CLASS(CMiniDockFrameWnd)))//IsFloating
    {
...
const BOOL CSizingControlBar::IsHorzDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);//AFX_IDW_DOCKBAR_BOTTOM
}

const BOOL CSizingControlBar::IsVertDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT ||
        m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT);//AFX_IDW_DOCKBAR_RIGHT
}
...
CSize CSizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
        ...
        nHitTest == HTTOPRIGHT || nHitTest == HTBOTTOMRIGHT))
    {
        ...
        if (nHitTest == HTTOPLEFT || nHitTest == HTBOTTOMLEFT)// HTTOPLEFT
        {
        ...
        if (nHitTest == HTTOPLEFT || nHitTest == HTTOPRIGHT)//HTTOPRIGHT  HTTOPLEFT
        {
...
void CSizingControlBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
    ...
    m_nDockBarID = GetParent()->GetDlgCtrlID();//GetDlgCtrlID
...
void CSizingControlBar::OnLButtonDown(UINT nFlags, CPoint point)
{
    if (m_pDockBar != NULL)
    {
        ...
        m_pDockContext->StartDrag(point);//StartDrag
    }
    else
        CWnd::OnLButtonDown(nFlags, point);//OnLButtonDown
}
...
void CSizingControlBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    if (m_pDockBar != NULL)
    {
        // toggle docking
        ASSERT(m_pDockContext != NULL);
        m_pDockContext->ToggleDocking();//ToggleDocking
    }
    else
        CWnd::OnLButtonDblClk(nFlags, point);//OnLButtonDblClk
}
...
void CSizingControlBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
    ...
    if (m_bTracking || IsFloating())//IsFloating
        return;
...
void CSizingControlBar::OnRButtonDown(UINT nFlags, CPoint point)
{
    ...
    baseCSizingControlBar::OnRButtonDown(nFlags, point);//OnRButtonDown
}
...
void CSizingControlBar::OnMouseMove(UINT nFlags, CPoint point)
{
    if (m_bTracking)
    {
        ...
        ClientToScreen(&ptScreen);//ClientToScreen

        OnTrackUpdateSize(ptScreen);//OnTrackUpdateSize
    }
                                     ...
                                     NCCALCSIZE_PARAMS FAR* lpncsp)
{
    ...
    if (!IsFloating() && m_pDockBar != NULL)
    {
        CSCBArray arrSCBars;
        int nThis;
        GetRowSizingBars(arrSCBars, nThis);//GetRowSizingBars

        BOOL bHorz = IsHorzDocked();//IsHorzDocked
        if (nThis > 0)
            m_dwSCBStyle |= bHorz ? SCBS_EDGELEFT : SCBS_EDGETOP;

        if (nThis < arrSCBars.GetUpperBound())//GetUpperBound
            m_dwSCBStyle |= bHorz ? SCBS_EDGERIGHT : SCBS_EDGEBOTTOM;
    }

    NcCalcClient(&lpncsp->rgrc[0], m_nDockBarID);//NcCalcClient
}
...
void CSizingControlBar::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    ...
    rc.DeflateRect(3, 5, 3, 3);//DeflateRect
    if (nDockBarID != AFX_IDW_DOCKBAR_FLOAT)
        rc.DeflateRect(2, 0, 2, 2);//DeflateRect

    switch(nDockBarID)
    {
    case AFX_IDW_DOCKBAR_TOP://AFX_IDW_DOCKBAR_TOP
        m_dwSCBStyle |= SCBS_EDGEBOTTOM;
        break;
    case AFX_IDW_DOCKBAR_BOTTOM://AFX_IDW_DOCKBAR_BOTTOM
        m_dwSCBStyle |= SCBS_EDGETOP;
        break;
    case AFX_IDW_DOCKBAR_LEFT://AFX_IDW_DOCKBAR_LEFT
        m_dwSCBStyle |= SCBS_EDGERIGHT;
        break;
    case AFX_IDW_DOCKBAR_RIGHT://AFX_IDW_DOCKBAR_RIGHT
    ...
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
        rc.DeflateRect(
            ...
            (m_dwSCBStyle & SCBS_EDGEBOTTOM) ? m_cxEdge : 0);//DeflateRect
...
void CSizingControlBar::OnNcPaint()
{
    ...
    GetClientRect(rcClient);//GetClientRect
    ClientToScreen(rcClient);//ClientToScreen
    GetWindowRect(rcBar);//GetWindowRect
    rcClient.OffsetRect(-rcBar.TopLeft());
    rcBar.OffsetRect(-rcBar.TopLeft());

    CDC mdc;
    mdc.CreateCompatibleDC(&dc);//CreateCompatibleDC
    ...
    CRect rcDraw = rcBar;
    DrawBorders(&mdc, rcDraw);//DrawBorders

    // erase the NC background
    mdc.FillRect(rcDraw, CBrush::FromHandle(
        (HBRUSH) GetClassLong(m_hWnd, GCL_HBRBACKGROUND)));//FillRect
    ...
    NcPaintGripper(&mdc, rcClient);//NcPaintGripper

    // client area is not our bussiness :)
    dc.IntersectClipRect(rcBar);//IntersectClipRect
    dc.ExcludeClipRect(rcClient);//ExcludeClipRect

    dc.BitBlt(0, 0, rcBar.Width(), rcBar.Height(), &mdc, 0, 0, SRCCOPY);//

    ReleaseDC(&dc);

    mdc.SelectObject(pOldBm);//SelectObject
    bm.DeleteObject();//DeleteObject
    mdc.DeleteDC();//
}
        ...
        for (int i = 0; i < 4; i++)
            if (GetEdgeRect(rcBar, GetEdgeHTCode(i), rcEdge))
                if (rcEdge.PtInRect(point))
                    return GetEdgeHTCode(i);//GetEdgeHTCode
...
void CSizingControlBar::StartTracking(UINT nHitTest, CPoint point)
{
    ...
    else
    {
        ...
        for (int i = 0; i < arrSCBars.GetSize(); i++)
        {
            ...
            if (i < nThis)
                m_nTrackPosMin -= nExcessWidth;//nExcessWidth
            else
                m_nTrackPosMax += nExcessWidth;//nExcessWidth
        }
    }
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ...
    else
    {
        ...
        for (i = nFirst; nDelta != 0 && i != nLimit; i += (bBefore ? -1 : 1))
        {
            pBar = arrSCBars[i];
...
void CSizingControlBar::OnTrackInvertTracker()
{
    ...
    rc.OffsetRect(-rcFrame.TopLeft());//TopLeft

    CDC *pDC = m_pDockSite->GetDCEx(NULL,
        DCX_WINDOW | DCX_CACHE | DCX_LOCKWINDOWUPDATE);//GetDCEx
    CBrush* pBrush = CDC::GetHalftoneBrush();
    CBrush* pBrushOld = pDC->SelectObject(pBrush);

    pDC->PatBlt(rc.left, rc.top, rc.Width(), rc.Height(), PATINVERT);//PatBlt
...
void CSizingControlBar::GetRowInfo(int& nFirst, int& nLast, int& nThis)
{
    ...
    nThis = m_pDockBar->FindBar(this);//FindBar
    ASSERT(nThis != -1);

    int i, nBars = m_pDockBar->m_arrBars.GetSize();//GetSize
...
void CSizingControlBar::GetRowSizingBars(CSCBArray& arrSCBars, int& nThis)
{
    ...
    for (int i = nFirstT; i <= nLastT; i++)
    {
        ...
        {
            if (pBar == this)
                nThis = arrSCBars.GetSize();//GetSize
...
BOOL CSizingControlBar::NegotiateSpace(int nLengthTotal, BOOL bHorz)
{
    ...
    CSCBArray arrSCBars;
    GetRowSizingBars(arrSCBars);//GetRowSizingBars
...
void CSizingControlBar::AlignControlBars()
{
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        pBar->GetWindowRect(&rc);//GetWindowRect
    ...
    if (bNeedRecalc)
        m_pDockSite->DelayRecalcLayout();//DelayRecalcLayout
}
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    UINT nID = GetDlgCtrlID();//GetDlgCtrlID
...
#endif
    m_rectFrameDragHorz.DeflateRect(2, 2);//m_rectFrameDragHorz
    m_rectFrameDragVert = m_rectFrameDragHorz;
    
    // adjust rectangles so that point is inside
    AdjustRectangle(m_rectDragHorz, pt);//Horz
    AdjustRectangle(m_rectDragVert, pt);//Vert
    AdjustRectangle(m_rectFrameDragHorz, pt);//DragHorz
    AdjustRectangle(m_rectFrameDragVert, pt);//DragHorz
...
BOOL CSCBMiniDockFrameWnd::Create(CWnd* pParent, DWORD dwBarStyle)
{
    ...
    if (!CMiniFrameWnd::CreateEx(dwExStyle,
        NULL, &afxChNil, dwStyle, rectDefault, pParent))//CreateEx
    {
...
void CSCBMiniDockFrameWnd::OnSize(UINT nType, int cx, int cy) 
{
    ...
    baseCSCBMiniDockFrameWnd::OnSize(nType, cx, cy);//size
}
...
#endif //_SCB_MINIFRAME_CAPTION
        lpMMI->ptMinTrackSize.x = r.Width();//¿í
        lpMMI->ptMinTrackSize.y = r.Height();//¸ß
    }
}
