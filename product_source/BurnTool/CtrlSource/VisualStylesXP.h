...
public:
	...
	HTHEME OpenThemeData(HWND hwnd, LPCWSTR pszClassList);//OpenThemeData
	HRESULT CloseThemeData(HTHEME hTheme);//CloseThemeData
	HRESULT DrawThemeBackground(HTHEME hTheme, HDC hdc, 
		int iPartId, int iStateId, const RECT *pRect, const RECT *pClipRect);//DrawThemeBackground
	HRESULT DrawThemeText(HTHEME hTheme, HDC hdc, int iPartId, 
		int iStateId, LPCWSTR pszText, int iCharCount, DWORD dwTextFlags, 
		DWORD dwTextFlags2, const RECT *pRect);//DrawThemeText
	HRESULT GetThemeBackgroundContentRect(HTHEME hTheme, HDC hdc, //GetThemeBackgroundContentRect
		int iPartId, int iStateId, const RECT *pBoundingRect, 
		RECT *pContentRect);
	HRESULT GetThemeBackgroundExtent(HTHEME hTheme, HDC hdc,
		int iPartId, int iStateId, const RECT *pContentRect, 
		RECT *pExtentRect);//GetThemeBackgroundExtent
	HRESULT GetThemePartSize(HTHEME hTheme, HDC hdc, 
		int iPartId, int iStateId, RECT * pRect, enum THEMESIZE eSize, SIZE *psz);//GetThemePartSize
	HRESULT GetThemeTextExtent(HTHEME hTheme, HDC hdc, 
		int iPartId, int iStateId, LPCWSTR pszText, int iCharCount, 
		DWORD dwTextFlags, const RECT *pBoundingRect, 
		RECT *pExtentRect);//GetThemeTextExtent
	HRESULT GetThemeTextMetrics(HTHEME hTheme, HDC hdc, 
		int iPartId, int iStateId, TEXTMETRIC* ptm);//GetThemeTextMetrics
	HRESULT GetThemeBackgroundRegion(HTHEME hTheme, HDC hdc, 
		int iPartId, int iStateId, const RECT *pRect, HRGN *pRegion);//GetThemeBackgroundRegion
	HRESULT HitTestThemeBackground(HTHEME hTheme, HDC hdc, int iPartId, 
		int iStateId, DWORD dwOptions, const RECT *pRect, HRGN hrgn, 
		POINT ptTest, WORD *pwHitTestCode);//HitTestThemeBackground
	HRESULT DrawThemeEdge(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, 
		const RECT *pDestRect, UINT uEdge, UINT uFlags, RECT *pContentRect);//DrawThemeEdge
	...
	HRESULT GetThemeMetric(HTHEME hTheme, HDC hdc, int iPartId, 
		int iStateId, int iPropId, int *piVal);//GetThemeMetric
	...
	HRESULT GetThemeBool(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, BOOL *pfVal);//GetThemeBool
	HRESULT GetThemeInt(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, int *piVal);//GetThemeInt
	HRESULT GetThemeEnumValue(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, int *piVal);//GetThemeEnumValue
	HRESULT GetThemePosition(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, POINT *pPoint);//GetThemePosition
	HRESULT GetThemeFont(HTHEME hTheme, HDC hdc, int iPartId, 
		int iStateId, int iPropId, LOGFONT *pFont);//GetThemeFont
	HRESULT GetThemeRect(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, RECT *pRect);//GetThemeRect
	HRESULT GetThemeMargins(HTHEME hTheme, HDC hdc, int iPartId, 
		int iStateId, int iPropId, RECT *prc, MARGINS *pMargins);//GetThemeMargins
	HRESULT GetThemeIntList(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, INTLIST *pIntList);//GetThemeIntList
	HRESULT GetThemePropertyOrigin(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, enum PROPERTYORIGIN *pOrigin);//GetThemePropertyOrigin
	...
	HRESULT GetThemeFilename(HTHEME hTheme, int iPartId, 
		int iStateId, int iPropId, LPWSTR pszThemeFileName, int cchMaxBuffChars);//GetThemeFilename
	COLORREF GetThemeSysColor(HTHEME hTheme, int iColorId);//GetThemeSysColor
	HBRUSH GetThemeSysColorBrush(HTHEME hTheme, int iColorId);//GetThemeSysColorBrush
	BOOL GetThemeSysBool(HTHEME hTheme, int iBoolId);//GetThemeSysBool
	int GetThemeSysSize(HTHEME hTheme, int iSizeId);//GetThemeSysSize
	HRESULT GetThemeSysFont(HTHEME hTheme, int iFontId, LOGFONT *plf);//GetThemeSysFont
	HRESULT GetThemeSysString(HTHEME hTheme, int iStringId, 
		LPWSTR pszStringBuff, int cchMaxStringChars);//GetThemeSysString
	HRESULT GetThemeSysInt(HTHEME hTheme, int iIntId, int *piValue);//GetThemeSysInt
	BOOL IsThemeActive();//IsThemeActive
	BOOL IsAppThemed();	//IsAppThemed
	HTHEME GetWindowTheme(HWND hwnd);//GetWindowTheme
	HRESULT EnableThemeDialogTexture(HWND hwnd, DWORD dwFlags);//EnableThemeDialogTexture
	BOOL IsThemeDialogTextureEnabled(HWND hwnd);//IsThemeDialogTextureEnabled
	DWORD GetThemeAppProperties();//GetThemeAppProperties
	void SetThemeAppProperties(DWORD dwFlags);//SetThemeAppProperties
	HRESULT GetCurrentThemeName(
		LPWSTR pszThemeFileName, int cchMaxNameChars, 
		LPWSTR pszColorBuff, int cchMaxColorChars,
		LPWSTR pszSizeBuff, int cchMaxSizeChars);//GetCurrentThemeName
	HRESULT GetThemeDocumentationProperty(LPCWSTR pszThemeName,
		LPCWSTR pszPropertyName, LPWSTR pszValueBuff, int cchMaxValChars);//GetThemeDocumentationProperty
	HRESULT DrawThemeParentBackground(HWND hwnd, HDC hdc, RECT* prc);//DrawThemeParentBackground
	HRESULT EnableTheming(BOOL fEnable);//EnableTheming

private:
	static HMODULE m_hThemeDll;
	static void* GetProc(LPCSTR szProc, void* pfnFail);//GetProc

	typedef HTHEME (__stdcall *PFNOPENTHEMEDATA)(HWND hwnd, LPCWSTR pszClassList);
	static HTHEME OpenThemeDataFail(HWND, LPCWSTR) {return NULL;}//OpenThemeDataFail

	typedef HRESULT(__stdcall *PFNCLOSETHEMEDATA)(HTHEME hTheme);//PFNCLOSETHEMEDATA
	static HRESULT CloseThemeDataFail(HTHEME) {return E_FAIL;}//CloseThemeDataFail
	...
	typedef BOOL (__stdcall *PFNISTHEMEACTIVE)();//PFNISTHEMEACTIVE
	static BOOL IsThemeActiveFail() {return FALSE;}//IsThemeActiveFail

	typedef BOOL(__stdcall *PFNISAPPTHEMED)();//PFNISAPPTHEMED
	static BOOL IsAppThemedFail() {return FALSE;}//IsAppThemedFail
