...
{
	if (m_hThemeDll != NULL)
		VERIFY( FreeLibrary(m_hThemeDll) );//FreeLibrary
	m_hThemeDll = NULL;
}
...
void* CVisualStylesXP::GetProc(LPCSTR szProc, void* pfnFail)
{
	void* pRet = pfnFail;
	if (m_hThemeDll != NULL)//
		pRet = GetProcAddress(m_hThemeDll, szProc);//GetProcAddress
	return pRet;
}
											 ...
											 LPWSTR pszSizeBuff, int cchMaxSizeChars)
{
	...
	if (pfn == NULL)
		pfn = (PFNGETCURRENTTHEMENAME)GetProc("GetCurrentThemeName", (void*)GetCurrentThemeNameFail);//GetProc
													   ...
													   LPWSTR pszValueBuff, int cchMaxValChars)
{
	...
	if (pfn == NULL)
		pfn = (PFNGETTHEMEDOCUMENTATIONPROPERTY)GetProc("GetThemeDocumentationProperty", (void*)GetThemeDocumentationPropertyFail);//GetProc
...
HRESULT CVisualStylesXP::DrawThemeParentBackground(HWND hwnd, HDC hdc, RECT* prc)
{
	...
	if (pfn == NULL)
		pfn = (PFNDRAWTHEMEPARENTBACKGROUND)GetProc("DrawThemeParentBackground", (void*)DrawThemeParentBackgroundFail);//GetProc
...
HRESULT CVisualStylesXP::EnableTheming(BOOL fEnable)
{
	...
	if (pfn == NULL)
		pfn = (PFNENABLETHEMING)GetProc("EnableTheming", (void*)EnableThemingFail);//GetProc
	return (*pfn)(fEnable);
}
