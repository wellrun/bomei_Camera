...
void CCoolTabCtrl::GetClientRect(LPRECT lpRect)
{
	...
	if(m_nStyle&TCS_DOWN)
	{
		...
		lpRect->right -= 2;//右
		lpRect->top += 2;//上
		lpRect->bottom -= ITEMBUTTON_HEIGHT;//下
	}
	else if(m_nStyle&TCS_UP)
	{
		lpRect->left += 2;//左
		lpRect->right -= 2;//右
		lpRect->top += ITEMBUTTON_HEIGHT;//上
		lpRect->bottom -= 2;//下
	}
}
...
{
	UINT width = pDC->GetTextExtent(m_sText).cx;//获取宽度
	if(m_hIcon)
		width += 18;//宽度加18
	return width + 9;//宽度加9
}
...
void CCoolTabCtrl::CPageItem::Draw(CDC *pDC, UINT nStyle, BOOL bActive)
{
	...
	if(nStyle&TCS_DOWN)
	{
		...
		pDC->MoveTo(rect.left,rect.top);//移动
	...
	///////////调整位置//////////
		...
		if (!m_sText.IsEmpty())//判断是否为空
		{
			///////////显示文字//////////
			rect.top += 5;
			CString sText = m_sText;
			int l = sText.GetLength();//获取长度
			int i;
			for(i=0;i<10 && pDC->GetTextExtent(sText).cx > rect.Width();i++,l-=2)
				sText = sText.Left(l-2);//左
			if(i > 0)
			{
				sText = sText.Left(l-2);//左边位置
				sText += "...";
			}
...
void CCoolTabCtrl::AutoSize()
{
	...
	if(m_nStyle&TCS_DOWN)
	{
		...
		rect.right -= 6;//右
		rect.bottom -= 1;//下
		rect.top = rect.bottom - ITEMBUTTON_HEIGHT + 1;// 上
	}
	else if(m_nStyle&TCS_UP)
	{
		rect.left += 2;//左
		rect.right -= 6;//右
		rect.bottom = rect.top + ITEMBUTTON_HEIGHT;//下
	}
	ItemRect = rect;
	int AreaWidth = 0,ItemMaxWidth,ItemIndex=0;
	ItemMaxWidth = rect.Width()/m_PageList.GetCount();//个数
	...
	for(pos=m_PageList.GetHeadPosition();pos!=NULL;ItemIndex++)
	{
		...
		{
			if(!bMonoSpace)
				ItemMaxWidth = pItem->GetAreaWidth(pDC);//获取空间大小
			AreaWidth += ItemMaxWidth;
			ItemRect.right = ItemRect.left+ItemMaxWidth-1;//右
			pItem->m_rect = ItemRect;
			ItemRect.left = ItemRect.right + 1;//左
			if(pItem->m_pWnd)
	...
	{
		ItemRect = rect;
		int MaxWidth = rect.Width()/PageCount;//宽度
		for(pos=m_PageList.GetHeadPosition();pos!=NULL;)
		{
			pItem=(CPageItem*)m_PageList.GetNext(pos);
			if(pItem)
			{
				...
				ItemMaxWidth = (ItemMaxWidth < AreaWidth)?MaxWidth:AreaWidth;//宽
				ItemRect.right = ItemRect.left+ItemMaxWidth;//右
				pItem->m_rect = ItemRect;
				ItemRect.left = ItemRect.right + 1;//左
			}
		}
...
void CCoolTabCtrl::SetActivePage(int nIndex)
{
	...
	if(m_nActivePage >= 0)
	{
		...
		if(pOldItem)
		{
			pOldItem->m_pWnd->ShowWindow(SW_HIDE);//是否显示
		}
	}
	...
	if(m_nStyle&TCS_ANIMATE)
		...
		pItem->m_pWnd->ShowWindow(SW_SHOW);//显示
}

void* CCoolTabCtrl::GetPageItem(UINT nIndex)
{
	CPageItem *pItem = NULL;
	POSITION pos = m_PageList.FindIndex(nIndex);//查找位置
	if(pos)
...
void CCoolTabCtrl::UpdateWindow()
{
 	...
 	if(m_nActivePage < 0)
 		...
 		SetActivePage(m_nActivePage);//自主页
 	Invalidate();
}
...
void CCoolTabCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	for(pos=m_PageList.GetHeadPosition();pos!=NULL;nItemIndex++)
	{
		pItem=(CPageItem*)m_PageList.GetNext(pos);//获取下一个
		if(pItem)
		{
			if(pItem->m_rect.PtInRect(point))//点
			{
...
void CCoolTabCtrl::OnTimer(UINT nIDEvent) 
{
	...
	if(pos)
	{
		...
		if(pItem)
		{
			...
			{
				rect.right = 10;//右
				rect.left = rect.right - width + 1;//左
				step = width/times;//步
				times--;//次数
				m_bEraseBkgnd = FALSE;//标志
				pItem->m_pWnd->ShowWindow(SW_SHOW);//显示
			}
			else
			{
				...
				{
					rect.right = width - times*step;//右
					rect.left = rect.right - width + 1;//左
					times--;//次数
				}
			}
			pItem->m_pWnd->MoveWindow(rect);//窗口移动
		}
	}
}
...
int CCoolTabCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	logFont.lfHeight = -12;//高
	logFont.lfWeight = 400;//宽
...
BOOL CCoolTabCtrl::DeleteAllPages()
{
	POSITION pos;
	CPageItem* pItem;//
	for(pos=m_PageList.GetHeadPosition();pos!=NULL;)//
