                               ...
                               DWORD dwStyle)
{
	...
	return baseCCoolBar::Create(lpszWindowName, pParentWnd, nID, dwStyle);//Create
}
...
void CCoolBar::NcPaintGripper(CDC* pDC, CRect rcClient)
{
    ...
    if (bHorz)
    {   // gripper at left
        gripper.left -= m_cyGripper;//左
        gripper.right = gripper.left + 3;//右
        gripper.top = rcbtn.bottom + 3;//上
    }
    else
    {   // gripper at top
        gripper.top -= m_cyGripper;//上
        gripper.bottom = gripper.top + 3;//下
        gripper.right = rcbtn.left - 3;//右
    }
    ...
    if (bHorz)
    {   // right side gripper
        rcGrip.left -= m_cyGripper + 1;//左
        rcGrip.right = rcGrip.left + 11;//右
        rcGrip.top = rcBtn.bottom + 3;//上
    }
    else
    {   // gripper at top
        rcGrip.top -= m_cyGripper + 1;//上
        rcGrip.bottom = rcGrip.top + 11;//下
        rcGrip.right = rcBtn.left - 3;//右
    }
        ...
        for (int i = 0; i < nSteps; i++)
        {
            ...
            if (bHorz)
            {
                r2.bottom = rcGrip.bottom - 
                    ((i * rcGrip.Height()) >> nShift);//下
                r2.top = rcGrip.bottom - 
                    (((i + 1) * rcGrip.Height()) >> nShift);//上
                if (r2.Height() > 0)
...
void CSCBButton::Paint(CDC* pDC)
{
    ...
    pDC->SetTextColor(clrOldTextColor);//设置色
}
...
BOOL CCoolBar::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	::SendMessage(m_hNotifyWnd, WM_NOTIFY, wParam, lParam);//发送消息
	*pResult = 0;
