...
void CPropPageFrameDefault::SetCaption(LPCTSTR lpszCaption, HICON hIcon /*= NULL*/)
{
	...
	// build image list
	if (m_Images.GetSafeHandle())
		m_Images.DeleteImageList();//m_Images
	if (hIcon)
	{
		ICONINFO	ii;
		...
		CBitmap	bmMask;
		bmMask.Attach(ii.hbmMask);//bmMask
		if (ii.hbmColor) DeleteObject(ii.hbmColor);

		BITMAP	bm;
		bmMask.GetBitmap(&bm);

		if (!m_Images.Create(bm.bmWidth, bm.bmHeight, ILC_MASK, 0, 1))//m_Images
			return;
...
CRect CPropPageFrameDefault::CalcMsgArea()
{
	...
	if (g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed())
	{
		...
		if (hTheme)
		{
			...
			if (GetShowCaption())
				rectContent.top = rect.top+GetCaptionHeight()+1;//top
			rect = rectContent;
		}
	}
	else if (GetShowCaption())
		rect.top+= GetCaptionHeight()+1;//top
...
CRect CPropPageFrameDefault::CalcCaptionArea()
{
	...
	if (g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed())
	{
		...
		if (hTheme)
		{
			...
			if (GetShowCaption())
				rectContent.bottom = rect.top+GetCaptionHeight();//bottom
			else
				rectContent.bottom = rectContent.top;//bottom
	...
	{
		if (GetShowCaption())
			rect.bottom = rect.top+GetCaptionHeight();//bottom
		else
...
BOOL CPropPageFrameDefault::OnEraseBkgnd(CDC* pDC) 
{
	if (g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed())
	{
		...
		if (hTheme)
		{
			...
			g_xpStyle.CloseThemeData(hTheme);//CloseThemeData
		}
		return TRUE;
	}
	else
	{
		return CWnd::OnEraseBkgnd(pDC);//OnEraseBkgnd
	}
}
