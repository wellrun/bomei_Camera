...
BOOL CTreePropSheet::SetPageIcon(CPropertyPage *pPage, LPCTSTR pszIconId)
{
	...
	pPage->m_psp.pszIcon = (LPCSTR)pszIconId;
	return TRUE;
}
...
{
	HICON	hIcon = Images.ExtractIcon(nImage);//ExtractIcon
	if (!hIcon)
		return FALSE;
...
void CTreePropSheet::MoveChildWindows(int nDx, int nDy)
{
	...
	while (pWnd)
	{
		...
		ScreenToClient(rect);//ScreenToClient
		rect.OffsetRect(nDx, nDy);//
		pWnd->MoveWindow(rect);//MoveWindow
...
void CTreePropSheet::RefillPageTree()
{
	...
	if (m_bTreeImages)
	{
		...
		if (pPageImages)
		{
			...
			{
				HICON	hIcon = pPageImages->ExtractIcon(nImage);//ExtractIcon
				m_Images.Add(hIcon);
				DestroyIcon(hIcon);
			}
		}
		...
		{	
			...
			// add default images
			hIcon = m_DefaultImages.ExtractIcon(0);//ExtractIcon
			if (hIcon)
			{
				m_Images.Add(hIcon);
				DestroyIcon(hIcon);
			}
			hIcon = m_DefaultImages.ExtractIcon(1);//ExtractIcon
			{
				m_Images.Add(hIcon);
				DestroyIcon(hIcon);
			}
	...
	for (int nPage = 0; nPage < nPageCount; ++nPage)
	{
		...
		pTabCtrl->GetItem(nPage, &ti);//GetItem
		strPagePath.ReleaseBuffer();//ReleaseBuffer

		// Create an item in the tree for the page
		HTREEITEM	hItem = CreatePageTreeItem(ti.pszText);//CreatePageTreeItem
		ASSERT(hItem);
		if (hItem)
		{
			m_pwndPageTree->SetItemData(hItem, nPage);//SetItemData

			// set image
			if (m_bTreeImages)
			{
				int	nImage = ti.iImage;
				if (nImage < 0 || nImage >= m_Images.GetImageCount())//GetImageCount
					nImage = m_DefaultImages.GetSafeHandle()? m_Images.GetImageCount()-1 : -1;

				m_pwndPageTree->SetItemImage(hItem, nImage, nImage);//SetItemImage
			}
		}
	}
}
...
HTREEITEM CTreePropSheet::CreatePageTreeItem(LPCTSTR lpszPath, HTREEITEM hParent /* = TVI_ROOT */)
{
	...
	while (hChild)
	{
		if (m_pwndPageTree->GetItemText(hChild) == strTopMostItem)//GetItemText
		{
			hItem = hChild;
			break;
		}
		hChild = m_pwndPageTree->GetNextItem(hChild, TVGN_NEXT);//GetNextItem
	}
...
//		m_pwndPageTree->SetItemData(hItem, (DWORD_PTR)-1);
	...
	if (strPath.IsEmpty())//IsEmpty
		...
		return CreatePageTreeItem(strPath, hItem);//CreatePageTreeItem
}
...
CString CTreePropSheet::SplitPageTreePath(CString &strRest)
{
	...
	CString	strItem(strRest.Left(nSeperatorPos));//Left
	strItem.Replace(_T("\\::"), _T("::"));//Replace
	strItem.Replace(_T("\\\\"), _T("\\"));//Replace
	strRest = strRest.Mid(nSeperatorPos+2);//strRest
	return strItem;
}
...
HTREEITEM CTreePropSheet::GetPageTreeItem(int nPage, HTREEITEM hRoot /* = TVI_ROOT */)
{
	...
	while (hItem)
	{
		if ((signed)m_pwndPageTree->GetItemData(hItem) == nPage)//m_pwndPageTree
			return hItem;
		if (m_pwndPageTree->ItemHasChildren(hItem))//ItemHasChildren
		{
		...
		hItem = m_pwndPageTree->GetNextItem(hItem, TVGN_NEXT);//hItem
	}
...
{
	HTREEITEM	hItem = GetPageTreeItem(nPage);//hItem
	if (!hItem)
		return FALSE;

	return m_pwndPageTree->SelectItem(hItem);//m_pwndPageTree
}
...
void CTreePropSheet::UpdateCaption()
{
	...
	if (m_bTreeImages)
	{
		...
		HICON	hIcon = m_Images.ExtractIcon(nImage);//hIcon
	...
	else if (bRealPage)
	{
		...
		if (pImages)
		{
			...
			if (pTabCtrl->GetItem((int)m_pwndPageTree->GetItemData(hItem), &ti))
				hIcon = pImages->ExtractIcon(ti.iImage);//hIcon

			m_pFrame->SetCaption(strCaption, hIcon);//m_pFrame
			if (hIcon)
				DestroyIcon(hIcon);
		}
		else
...
void CTreePropSheet::ActivatePreviousPage()
{
	...
	else
	{
		...
		if (!hPrevItem)
		{
			...
			for (;;)
			{
				while (m_pwndPageTree->GetNextSiblingItem(hPrevItem))//GetNextSiblingItem
					hPrevItem = m_pwndPageTree->GetNextSiblingItem(hPrevItem);//

				if (m_pwndPageTree->ItemHasChildren(hPrevItem))//ItemHasChildren
					hPrevItem = m_pwndPageTree->GetChildItem(hPrevItem);//
				else
					break;
			}
...
void CTreePropSheet::ActivateNextPage()
{
	...
	else
	{
		...
		// the page index, because we won't skip empty pages.
		// so we have to walk the page tree
		HTREEITEM	hItem = m_pwndPageTree->GetSelectedItem();//GetSelectedItem
		ASSERT(hItem);
		if (!hItem)
			return;

		HTREEITEM	hNextItem = NULL;
		if ((hNextItem = m_pwndPageTree->GetChildItem(hItem)) != NULL)//GetChildItem
			;
		else if ((hNextItem = m_pwndPageTree->GetNextSiblingItem(hItem)) != NULL)//GetNextSiblingItem
			;
		else if (m_pwndPageTree->GetParentItem(hItem))//GetParentItem
		{
			while (!hNextItem)
			{
				hItem = m_pwndPageTree->GetParentItem(hItem);//GetParentItem
				if (!hItem)
					break;

				hNextItem	= m_pwndPageTree->GetNextSiblingItem(hItem);//GetNextSiblingItem
			}
		}
...
BOOL CTreePropSheet::OnInitDialog() 
{
	if (m_bTreeViewMode)
	{
		...
		if (m_DefaultImages.GetSafeHandle())
		{
			...
			m_Images.Create(ii.rcImage.right-ii.rcImage.left, ii.rcImage.bottom-ii.rcImage.top, ILC_MASK, 0, 1);
		}
		else
			m_Images.Create(16, 16, ILC_MASK, 0, 1);//Create
	}
...
//	HighColorTab::UpdateImageList(*this);
	...
	const int nTreeWidth = m_nPageTreeWidth;//宽
	const int nTreeSpace = 5;

	CRect rectSheet;
	GetWindowRect(rectSheet);
	rectSheet.right += nTreeWidth;//右
	...
	rectTree.right = rectTree.left + nTreeWidth - nTreeSpace;//右
	...
	if (!m_bPageCaption)
	{
		...
		ScreenToClient(rectFrame);
		rectFrame.top+= rectFrameCaption.Height();//上
		...
		rectTree.bottom-= rectFrameCaption.Height();//下

		// make us smaller
		CRect	rect;
		GetWindowRect(rect);
		rect.top+= rectFrameCaption.Height()/2;//上
		rect.bottom-= rectFrameCaption.Height()-rectFrameCaption.Height()/2;//下
		if (GetParent())
...
LRESULT CTreePropSheet::OnAddPage(WPARAM wParam, LPARAM lParam)
{
	...
	RefillPageTree();//RefillPageTree
	SelectCurrentPageTreeItem();//SelectCurrentPageTreeItem
...
void CTreePropSheet::OnPageTreeSelChanging(NMHDR *pNotifyStruct, LRESULT *plResult)
{
	*plResult = 0;
	...
	int					nPage = m_pwndPageTree->GetItemData(pTvn->itemNew.hItem);//GetItemData
	BOOL				bResult;
	if (nPage<0 || (unsigned)nPage>=m_pwndPageTree->GetCount())//GetCount
		bResult = KillActiveCurrentPage();//KillActiveCurrentPage
	else
		bResult = SetActivePage(nPage);//SetActivePage
	...
	m_pwndPageTree->SetFocus();//SetFocus
...
LRESULT CTreePropSheet::OnIsDialogMessage(WPARAM wParam, LPARAM lParam)
{
	...
	if (pMsg->message==WM_KEYDOWN && pMsg->wParam==VK_TAB && GetKeyState(VK_CONTROL)&0x8000)
	{
		if (GetKeyState(VK_SHIFT)&0x8000)
			ActivatePreviousPage();//ActivatePreviousPage
		else
			ActivateNextPage();//ActivateNextPage
		return TRUE;
	}
