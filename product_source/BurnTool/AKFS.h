...
public:
	...
	BOOL GetDriverInfo(UINT nID, UINT StartID, UINT DriverCnt, UINT *DriverNum, UCHAR *Info, UINT MediumType);
	BOOL Init(VOID);
	BOOL DownloadImg(UINT nID, HANDLE hFile, T_IMG_INFO *img_info, UINT img_buf_len);
	BOOL Malloc_Burn_nID_Idex(UINT nID);
	BOOL Free_Burn_nID_Idex(UINT nID, T_U8  MediumType);
	CAKFS();
	virtual ~CAKFS();
