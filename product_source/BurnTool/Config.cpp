...
TCHAR *g_strChipName[] = {
   ...
   _T("AK_10XXC"), _T("AK_39XX"), _T("AK_37XX_L"), _T("AK_37XX_C"),_T("RESERVER"), NULL
};
...
void CConfig::DefaultConfig()
{
	...
	memset(path_nandretry, 0, sizeof(path_nandboot_new));
	...
	_tcscpy(path_nandretry, _T("nandretry.bin"));
	...
	memset(g_OTP_serial_current, 0, sizeof(g_OTP_serial_current));          //全局MAC低位
	...
	_tcscpy(OTP_serial, _T("0000000000000001"));
	_tcscpy(OTP_serial_current, OTP_serial);

	macaddr_flag = FALSE;         //MAC标志
	sequenceaddr_flag = FALSE;    //序列号标志
	OTP_serial_flag = FALSE;
...
BOOL CConfig::ReadConfig(LPCTSTR file_path)
{
	...
	while(read_len > 0)
	{
		...
		else if(_T("path") == subLeft.Left(4))
		{
			...
			else if(_T("path nand retry") == subLeft)
			{
				_tcsncpy(path_nandretry, subRight, MAX_PATH);
			}
...
/*			else if(_T("ram control address") == subLeft)
		...
		*/
		...
		else if(_T("otp serial flag") == subLeft)//强制标志
		{
			OTP_serial_flag = atoi(T2A(subRight));
		}
		else if(_T("otp serial num") == subLeft)//序列号结束的高位
		{
			memset(tempbuf, 0, MAX_MAC_SEQU_ADDR_COUNT+1);
			memset(surtempbuf, 0, MAX_MAC_SEQU_ADDR_COUNT+1);
			_tcscpy(surtempbuf, subRight);
			...
			_tcsncpy(OTP_serial, tempbuf, MAX_MAC_SEQU_ADDR_COUNT);
			if (_tcscmp (OTP_serial, OTP_serial_current) > 0 )
			{
				_tcscpy(OTP_serial_current, OTP_serial);
			}
		}
...
BOOL CConfig::WriteConfig(LPCTSTR file_path)
{
	...
	pFile->WriteString(str);
	str.Format(_T("path nand retry = %s\r\n"), path_nandretry);
	pFile->WriteString(str);
...
//	pFile->WriteString(str);
	...
	*/
	...
	str.Format(_T("otp serial flag = %d\r\n"), OTP_serial_flag);
	pFile->WriteString(str);
	str.Format(_T("otp serial num = %s\r\n"), OTP_serial_current);  //记录当前已用到的序号
	pFile->WriteString(str);
	/*
...
BOOL CConfig::read_config_addr(LPCTSTR file_path, TCHAR *pBufAddr, TCHAR *pGAddr, TCHAR *pCurAddr)
{
	...
	for (j=0; j < 32; j++)
	{
		...
		if (tmpBuf[0] == 0)
		{
			if (j == 0)
			{
				...
				if (pBufAddr == otpserialcurrent)
				{
					_tcscpy(pCurAddr, OTP_serial);//记录汉前的序列号
				}
				return FALSE;
			}
	...
	else if (pBufAddr == otpserialcurrent)
	{
		//地址递增一
		Otp_serial_add(tmpBufMax, pCurAddr);
		if (_tcscmp (OTP_serial, pCurAddr) > 0 )
		{
			_tcscpy(pCurAddr, OTP_serial);
		}
	}
	else
	{
