...
//fsa获取MEDIUM
T_pVOID Test_FSA_GetImgMedium(T_U8 driverID, T_U32 burn_ID)
{
    DRIVER_INFO DriverInfo = {0};
    
    if (!FS_GetDriver_Burn(&DriverInfo, driverID, burn_ID))
    {
        return 0;
    }
...
BOOL CAKFS::Malloc_Burn_nID_Idex(UINT nID)
{
	if (!FS_Malloc_Burn_nID_Idex(nID))
	{
		return FALSE;
	}
	return TRUE;
...
BOOL CAKFS::Free_Burn_nID_Idex(UINT nID, T_U8  MediumType)
{
	if (!FS_Free_Burn_nID_Idex(nID,  MediumType))
	{
		return FALSE;
	}
	return TRUE;
...
//获取分区信息
BOOL CAKFS::GetDriverInfo(UINT nID, UINT StartID, UINT DriverCnt,  UINT *DriverNum, UCHAR *Info, UINT MediumType)
{
	UINT i;
    ...
    *DriverNum = 0;
	...
	{
		//通过id获取相应的分区
		if (FS_GetDriver_Burn(&Driver_Info, StartID+i, nID))
		{
            (*DriverNum)++;
...
BOOL CAKFS::DownloadImg(UINT nID, HANDLE hFile, T_IMG_INFO *img_info, UINT img_buf_len)
{
	...
	//此接口返回一个结构体，
	//使用时要进行转换，库内部实现
	handle = FSA_write_image_begin_burn(img_info, nID);
