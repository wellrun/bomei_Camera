...
void Burn_otp_serial_Fail(UINT nID, TCHAR *Otpserialbuf);
					 ...
					 PTCHAR seriallowbuf, PTCHAR serialtempbuf, BOOL *readSerialflag, UINT  *Serialmode)
{
	...
	if(theConfig.sequenceaddr_flag && nID == 1) 
	{
		BYTE buf[MAX_MAC_SEQU_ADDR_COUNT*2+1] = {0};
		CLogFile  burnFile(file_name);
		...
		if (theConfig.fore_write_serial_addr)
		{
			*Serialmode = TRUE;
			...
			_tcscpy(SerialBuf, theConfig.sequence_current);
		}
		else
		{
			*Serialmode = FALSE;
		...
		//显示当前通道的序列号地址
		_tcscpy(theConfig.g_sequence_show_current[nID-1], SerialBuf);
...
//序列号地址加1
BOOL Otp_serial_Addr_1(TCHAR *buf, TCHAR *buf_temp)
{
	CString str;
	char temp_buf[30] = {0};
	...
	if (!theConfig.Otp_serial_add(buf_temp, theConfig.OTP_serial_current))
	{
		return FALSE;
	}
...
BOOL Get_OTP_serial(TCHAR *buf, UINT channelID)
{
	...
	if (theConfig.g_OTP_serial_current[channelID-1][0] != 0)
	{
		memcpy(buf, theConfig.g_OTP_serial_current[channelID-1], MAX_MAC_SEQU_ADDR_COUNT);
		memset(theConfig.g_OTP_serial_current[channelID-1], 0, MAX_MAC_SEQU_ADDR_COUNT+1);
	}
	else
	{
		for (i=0; i<32; i++)
		{
			//判断是否有些通过在上一次有烧录失败过
			if (theConfig.g_OTP_serial_current[i][0] != 0)
			{
				memcpy(buf, theConfig.g_OTP_serial_current[i], MAX_MAC_SEQU_ADDR_COUNT);
				memset(theConfig.g_OTP_serial_current[i], 0, MAX_MAC_SEQU_ADDR_COUNT+1);
				theConfig.read_currentdevicenum_addr(CONFIG_ADDR_FILE_NAME, otpserialcurrent, NULL, tempcurrnetbuf, channelID-1);
				theConfig.write_config_addr(CONFIG_ADDR_FILE_NAME, otpserialcurrent, passY, tempcurrnetbuf, i);
				theConfig.write_config_addr(CONFIG_ADDR_FILE_NAME, otpserialcurrent, passN, buf, channelID-1);
		...
		if (!Otp_serial_Addr_1(buf, theConfig.OTP_serial_current))
		{
			ReleaseMutex(theConfig.m_hMutGetAddr);
			return FALSE;
		}
...
//写序列号到安全区
BOOL Burn_Write_OTP_SERIAL(UINT nID, HWND hWnd, PTCHAR file_name, PTCHAR Otpserialbuf)
{
	USES_CONVERSION;
	if(theConfig.OTP_serial_flag && theConfig.burn_mode == E_CONFIG_SPI_NAND && theConfig.chip_type == CHIP_37XX_C) 
	{
		TCHAR buf[MAX_MAC_SEQU_ADDR_COUNT+1] = {0};
		BYTE tbuf[MAX_MAC_SEQU_ADDR_COUNT*2+1] = {0};
		UINT *ptr = (UINT *)tbuf; 
		CLogFile  burnFile(file_name);
		LONG64 tempsequence = 0;
		UINT tbuf_len = 16;
		UINT i = 0;

		
		//先进行读取otp区数据，判断是否有数据，如果有，就不进行烧录，如果没有，那就可行烧录
		//读OPT，只读取开头的16字节就可以了
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_READ_OTP_SERIAL));
#if 0
		if (BT_Read_OTP_Serial(nID, tbuf, tbuf_len) != BT_SUCCESS)
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_READ_OTP_SERIAL_FAIL));
			return FALSE;
		}

		for (i = 0; i < tbuf_len; i++)
		{
			if (tbuf[i] != 0)
			{
				break;
		...
		if (i != tbuf_len)
		{
			if (!Get_OTP_serial(buf, nID))//
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_OTP_SERIAL_FAIL));
				return FALSE;
			}
			
			_tcscpy(Otpserialbuf, buf);
			memcpy(tbuf, T2A(buf), tbuf_len*2);
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_OTP_SERIAL));
			
			if (BT_Write_OTP_Serial(nID, tbuf, tbuf_len) != BT_SUCCESS)
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_OTP_SERIAL_FAIL));
				return FALSE;
			}
			theConfig.write_config_addr(CONFIG_ADDR_FILE_NAME, otpserialcurrent, passY, buf, nID-1);
		}
		else
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_OTP_SERIAL_EXIST));
		}
		//显示当前通道的序列号地址
		_tcscpy(theConfig.g_OTP_serial_show_current[nID-1], buf);
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_BURN_OTP_SERIAL_SHOW));
						  ...
						  BOOL readSerialflag, UINT  Serialmode)
{
	...
	if(theConfig.sequenceaddr_flag && nID == 1) 
	{
		...
		if (BT_WriteASAFile(nID, "SERADDR", tbuf, ptr[0]+4, Serialmode) != BT_SUCCESS)
		{
			//Burn_Fail(seriallowbuf, maclowbuf, nID, readSerialflag, readmacflag);
			burnFile.WriteLogFile(LOG_LINE_TIME, "->fail!\r\n");
...
BOOL Download_Channel_Addranddata_ToRam(UINT nID, HWND hWnd, TCHAR *file_name)
{
	...
	else if(theConfig.chip_type == CHIP_37XX || theConfig.chip_type == CHIP_37XX_L || theConfig.chip_type == CHIP_37XX_C)//ak37芯片
	{
...
{
	if ((theConfig.planform_tpye == E_ROST_PLANFORM) && ((CHIP_37XX == theConfig.chip_type)  || (CHIP_37XX_L == theConfig.chip_type) || theConfig.chip_type == CHIP_37XX_C))
	{
		CLogFile  burnFile(file_name);
...
BOOL Burn_SwitchUSB11To20(UINT nID, HWND hWnd, PTCHAR file_name)
{
	...
	{
		if (((theConfig.bUsb2) && (CHIP_1080L != theConfig.chip_type) && (CHIP_10XXC != theConfig.chip_type) && (CHIP_37XX_L != theConfig.chip_type) && (theConfig.chip_type != CHIP_37XX_C))
			|| ((theConfig.bPLL_Frep_change) && ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type) || (CHIP_37XX_L == theConfig.chip_type) || (CHIP_37XX_C == theConfig.chip_type))))
		{
			CLogFile  burnFile(file_name);
...
BOOL Burn_GetDiskInfo (UINT nID, UINT StartID, UINT DriverCnt,  UINT *DriverNum, T_DRIVER_INFO* pDriverInfo, UCHAR MediumType)
{
	CAKFS cAK;

	return cAK.GetDriverInfo(nID, StartID, DriverCnt, DriverNum, (UCHAR *)pDriverInfo, MediumType);
}
//在线制作镜像
			...
			|| (CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type))
		{
			...
			if (CHIP_1080L == theConfig.chip_type || CHIP_10XXC == theConfig.chip_type)
			{
				...
				Save_udiskfile_drivernum();
					...
					if (!Burn_GetDiskInfo(nID, StartID, DriverCnt, &driverNum, (T_DRIVER_INFO*)pDriverInfoBuf, MediumType))
				{
...
BOOL Burn_DownloadBoot(UINT nID, HWND hWnd, PTCHAR file_name)
{
	...
	if (CHIP_37XX_L == theConfig.chip_type)
	{
		...
		{
			if(BT_Get_nandretry_path(theApp.ConvertAbsolutePath(theConfig.path_nandretry)) != BT_SUCCESS)
			{
				burnFile.WriteLogFile(LOG_LINE_TIME, "->BT_Get_nandretry_path fail!\r\n");
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_DOWNLOAD_BOOT_FAIL));
				return FALSE;
			}
		}
...
//umount nandflash
BOOL Burn_FreeMedium(UINT nID, HWND hWnd)
{
	if ((theConfig.planform_tpye == E_ROST_PLANFORM) && ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type)))
	{
		CAKFS cAK;
		if (E_CONFIG_NAND == theConfig.burn_mode)
		{
			if (!cAK.Free_Burn_nID_Idex(nID, FS_NAND))
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_LOW_FORMAT_FAIL));
				return FALSE;
			}
		}
		else if (E_CONFIG_SD == theConfig.burn_mode)
		{
			if (!cAK.Free_Burn_nID_Idex(nID, FS_SD))
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_LOW_FORMAT_FAIL));
				return FALSE;
			}
...
//	burnFile->WriteLogFile(LOG_LINE_TIME, "Burn_GetRegValue fail\n");
	...
	//区分是37还是37L
	if (theConfig.chip_type == CHIP_37XX || theConfig.chip_type == CHIP_37XX_L || theConfig.chip_type == CHIP_37XX_C)
	{
		...
		else if (0x20130200 == metafixValue)
		{
			theConfig.chip_type = CHIP_37XX_C;
		}
		else
		{
...
BOOL Burn_unmountdisk_Wait(UINT nID, UINT StartID, T_PMEDIUM Pmedium)
{
	if ((theConfig.planform_tpye == E_ROST_PLANFORM) && ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type)))
	{
		theConfig.run_time++;
		...
		if (!theConfig.bAuto_Burnflag)
		{
			if (theConfig.run_time == theConfig.burn_totalnum)
			{
				theConfig.burn_totalnum = 0;
				SetEvent(all_mount_event);  //mount所有通道后，释放所有的
			}
			
			//等待所以线程都mount完盘
			if(WaitForSingleObject(all_mount_event, 3600000) != WAIT_OBJECT_0)
			{
				return FALSE;
			}
		}
...
BOOL BurnThread(UINT nID)
{
	...
	TCHAR Otpserialbuf[MAX_MAC_SEQU_ADDR_COUNT+1] = {0};
	UINT  Serialmode = 0;
	UINT com_mode = 0;
	CString str;
...
//	char *timeline;
	...
	*/
	if (!theConfig.bAuto_Burnflag && theConfig.bUDiskUpdate && m_budisk_getUSBnum)
	{
		WaitForSingleObject(g_handle,INFINITE);
    ...
    if (!theConfig.bAuto_Burnflag && m_worknum == 0)
    {
	...
	//传输平台和芯片的信息
	BT_SetTranscParam(nID, &transc_para);
...
SET_RESV_AREA:
	...
	if (!Burn_unmountdisk_Wait(nID,  StartID, Pmedium))
	{
		burnFile.WriteLogFile(LOG_LINE_TIME, "Burn_unmountdisk_Wait fail!\r\n" );
		Burn_Fail(seriallowbuf, maclowbuf, nID, readSerialflag, readmacflag);
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_LOW_FORMAT_FAIL));
		Burn_FreeMedium(nID,  hWnd);
		Burn_Free_Medium(Pmedium);
...
*/
		//umount分区
		//释放muount库里创建的内存
		if (!Burn_FreeMedium(nID,  hWnd))
		{
			burnFile.WriteLogFile(LOG_LINE_TIME, "Burn_FreeMedium fail!\r\n" );
			return FALSE;
		}
...
BURN_BOOT:
	...
	//烧录OTP序列号
	if (!Burn_Write_OTP_SERIAL(nID, hWnd, file_name, Otpserialbuf))
	{
		burnFile.WriteLogFile(LOG_LINE_TIME, "Burn_WriteSERIALInfo fail!\r\n" );
		Burn_Fail(seriallowbuf, maclowbuf, nID, readSerialflag, readmacflag);
		Burn_otp_serial_Fail(nID, Otpserialbuf);
		return FALSE;
	}
...
BURN_SUCCESS:
	...
	if (m_worknum != 0)
    {
		...
		burnFile.WriteLogFile(LOG_LINE_TIME, "end work number = %d\n", m_worknum);
        if (0 == m_worknum)
        {
			//当所以通首的都完成后，再进行设置事件
...
void Burn_otp_serial_Fail(UINT nID, TCHAR *Otpserialbuf)
{
	if(!theConfig.bUpdate && theConfig.OTP_serial_flag) 
	{
		if(WaitForSingleObject(theConfig.m_hMutGetAddr, 5000) != WAIT_OBJECT_0)
		{
			ReleaseMutex(theConfig.m_hMutGetAddr);
		...
		if (Otpserialbuf != NULL && theConfig.g_OTP_serial_current[nID - 1][0] == NULL)
		{
			//烧录失败时记录此通过的值
			_tcscpy(theConfig.g_OTP_serial_current[nID-1], Otpserialbuf);
		}
		ReleaseMutex(theConfig.m_hMutGetAddr);

		//烧录失败时记录此通过的值到文档上
		theConfig.write_config_addr(CONFIG_ADDR_FILE_NAME, otpserialcurrent, passN, Otpserialbuf, nID-1);
	}
}
//烧录失败
void Burn_Fail(TCHAR *serialtempbuf, TCHAR *mactempbuf, UINT nID, BOOL readSerialflag, BOOL readmacflag)
...
BOOL Serialaddr_isunuse(char *buf, int len)
{
	...
	memcpy(temphuf, buf, MAX_MAC_SEQU_ADDR_COUNT);
