...
void CPageGeneral::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MaxChars(pDX, m_edit_sequence_start_addr_high, 32);
	//}}AFX_DATA_MAP
}

int ctl_array[41] = {IDC_STATIC_k1,IDC_CHECK_MAC_ADDR,IDC_CHECK_FORCE_WRITE_MAC_ADDR,IDC_MACADDR_RESET,
					...
					IDC_STATIC_SERIAL_START_NAME,IDC_EDIT_SEQUENCE_START_ADDR_HIGH,
					IDC_CHECK_OTP_SERIAL,IDC_STATIC_OTP_SERIAL,IDC_EDIT_OTP_SERIAL,
					...
					IDC_STATIC_PATH_NANDRETRY,IDC_EDIT_PATH_NANDRETRY,IDC_BTN_BROWSE_NANDRETRY,
...
BEGIN_MESSAGE_MAP(CPageGeneral, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_BTN_BROWSE_NANDRETRY, OnBtnBrowseNandretry)
	ON_BN_CLICKED(IDC_CHECK_OTP_SERIAL, OnCheckOtpSerial)
	ON_EN_CHANGE(IDC_EDIT_OTP_SERIAL, OnChangeEditOtpSerial)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
	if ((theConfig.chip_type == CHIP_37XX || theConfig.chip_type == CHIP_37XX_L) && (theConfig.burn_mode != E_CONFIG_SPI_NAND))
	{
		GetDlgItem(IDC_STATIC_PATH_NANDBOOT_NEW)->ShowWindow(TRUE);//可用
		GetDlgItem(IDC_EDIT_PATH_NANDBOOT_NEW)->ShowWindow(TRUE);//可用
		GetDlgItem(IDC_BTN_BROWSE_NANDBOOT_NEW)->ShowWindow(TRUE);//可用
	}
	else
	{
	...
	if ((theConfig.chip_type == CHIP_37XX || theConfig.chip_type == CHIP_37XX_L) 
		&& (theConfig.burn_mode == E_CONFIG_NAND))
	{
		GetDlgItem(IDC_STATIC_PATH_NANDRETRY)->ShowWindow(TRUE);//可用
		GetDlgItem(IDC_EDIT_PATH_NANDRETRY)->ShowWindow(TRUE);//可用
		GetDlgItem(IDC_BTN_BROWSE_NANDRETRY)->ShowWindow(TRUE);//可用
	}
	else
	{
		GetDlgItem(IDC_STATIC_PATH_NANDRETRY)->ShowWindow(FALSE);//不可用
		GetDlgItem(IDC_EDIT_PATH_NANDRETRY)->ShowWindow(FALSE);//不可用
		GetDlgItem(IDC_BTN_BROWSE_NANDRETRY)->ShowWindow(FALSE);//不可用
	}
...
*/
	{	
		if ((theConfig.planform_tpye == E_LINUX_PLANFORM) 
			...
			&& (theConfig.chip_type == CHIP_37XX || theConfig.chip_type == CHIP_37XX_L || theConfig.chip_type == CHIP_37XX_C)))
		{
	...
	if ((theConfig.chip_type == CHIP_37XX_C || theConfig.chip_type == CHIP_37XX) && theConfig.burn_mode == E_CONFIG_SPI_NAND)
	{
		GetDlgItem(IDC_CHECK_OTP_SERIAL)->ShowWindow(FALSE);//
		GetDlgItem(IDC_CHECK_OTP_SERIAL)->EnableWindow(FALSE);//
		GetDlgItem(IDC_STATIC_OTP_SERIAL)->ShowWindow(TRUE);//
		GetDlgItem(IDC_STATIC_OTP_SERIAL)->EnableWindow(TRUE);//
		GetDlgItem(IDC_EDIT_OTP_SERIAL)->ShowWindow(TRUE);//
		GetDlgItem(IDC_EDIT_OTP_SERIAL)->EnableWindow(TRUE);//
	} 
	else
	{
		GetDlgItem(IDC_CHECK_OTP_SERIAL)->ShowWindow(FALSE);//
		GetDlgItem(IDC_CHECK_OTP_SERIAL)->EnableWindow(FALSE);//
		GetDlgItem(IDC_STATIC_OTP_SERIAL)->ShowWindow(FALSE);//
		GetDlgItem(IDC_STATIC_OTP_SERIAL)->EnableWindow(FALSE);//
		GetDlgItem(IDC_EDIT_OTP_SERIAL)->ShowWindow(FALSE);//
		GetDlgItem(IDC_EDIT_OTP_SERIAL)->EnableWindow(FALSE);//
		((CButton *)GetDlgItem(IDC_CHECK_OTP_SERIAL))->SetCheck(FALSE);
	}
	...
	//otp序列号地址
	check = ((CButton *)GetDlgItem(IDC_CHECK_OTP_SERIAL))->GetCheck();	
    if (check)
    {
		GetDlgItem(IDC_STATIC_OTP_SERIAL)->EnableWindow(TRUE);//序列号
        GetDlgItem(IDC_EDIT_OTP_SERIAL)->EnableWindow(TRUE);//
    }
    else
    {	
...
BOOL CPageGeneral::get_config_data(CConfig &config)
{
	...
	GetDlgItemText(IDC_EDIT_PATH_NANDRETRY, str);
	_tcsncpy(config.path_nandretry, str, MAX_PATH);
...
//	_tcsncpy(config.path_bios, str, MAX_PATH);
	...
	theConfig.lower_to_upper(surBuf, dstBuf);
	_tcscpy(config.sequence_current, dstBuf);
	memset(surBuf, 0, MAX_MAC_SEQU_ADDR_COUNT+1);
	memset(dstBuf, 0, MAX_MAC_SEQU_ADDR_COUNT+1);
	

	//读取otp序列号
	GetDlgItemText(IDC_EDIT_OTP_SERIAL, str);
	_tcsncpy(surBuf, str, MAX_MAC_SEQU_ADDR_COUNT);
	//把读出sequence_end_high地址转换出大写
	theConfig.lower_to_upper(surBuf, dstBuf);
	_tcscpy(config.OTP_serial, dstBuf);
	if (_tcscmp (config.OTP_serial, config.OTP_serial_current) >= 0)//比较
	{
		_tcsncpy(config.OTP_serial_current, config.OTP_serial, MAX_MAC_SEQU_ADDR_COUNT);
	}
    ...
    if (check)
    {
		theConfig.OTP_serial_flag = TRUE;
	}
	else
	{
		theConfig.OTP_serial_flag = FALSE;
	}
...
BOOL CPageGeneral::set_config_item(CConfig &config)
{
	...
	SetDlgItemText(IDC_EDIT_PATH_NANDRETRY, config.path_nandretry);
...
*/
	...
	str.Format(_T("%s"), theConfig.sequence_current);
	GetDlgItem(IDC_EDIT_SEQUENCE_START_ADDR_HIGH)->SetWindowText(str);//

	//otp
	((CButton *)GetDlgItem(IDC_CHECK_OTP_SERIAL))->SetCheck(theConfig.OTP_serial_flag);
	GetDlgItem(IDC_EDIT_OTP_SERIAL)->EnableWindow(theConfig.OTP_serial_flag);//
	GetDlgItem(IDC_STATIC_OTP_SERIAL)->EnableWindow(theConfig.OTP_serial_flag);//
	str.Format(_T("%16s"), theConfig.OTP_serial);
	GetDlgItem(IDC_EDIT_OTP_SERIAL)->SetWindowText(str);//
...
void CPageGeneral::SetupDisplay()
{
	...
	str = theApp.GetString(IDS_GENERAL_BURN_OTP_SEQUNCE);
	GetDlgItem(IDC_CHECK_OTP_SERIAL)->SetWindowText(str);
	str = theApp.GetString(IDS_GENERAL_STATIC_OTP_SEQUNCE);
	GetDlgItem(IDC_STATIC_OTP_SERIAL)->SetWindowText(str);
...
//	GetDlgItem(IDC_STATIC_PATH_BIOS)->SetWindowText(str);
	...
	str = theApp.GetString(IDS_GENERAL_EXPLORER);
	GetDlgItem(IDC_BTN_BROWSE_NANDRETRY)->SetWindowText(str);
...
void CPageGeneral::OnChkConfigCmdline() 
{
    ...
    check = ((CButton *)GetDlgItem(IDC_CHK_CONFIG_CMDLINE))->GetCheck();
	theConfig.cmdLine.bCmdLine = check;
    if (check)
    {
        GetDlgItem(IDC_EDIT_CMDLINE_ADDR)->EnableWindow(TRUE);
        GetDlgItem(IDC_EDIT_CMDLINE_DATA)->EnableWindow(TRUE);
...
void CPageGeneral::OnChangeEditSequenceStartAddrHigh() 
{
	...
	// TODO: Add your control notification handler code here
	limit_serial_addr_input(IDC_EDIT_SEQUENCE_START_ADDR_HIGH, 32, theConfig.sequence_current);
}
...
void CPageGeneral::OnBtnBrowseNandretry() 
{
	// TODO: Add your control notification handler code here
	OPENFILENAME ofn;
	TCHAR pstrFileName[260] = {0}, pstrTitleName[260] = {0};
	...
	if(GetOpenFileName (&ofn))
	{
		...
		{
			relative_path = pstrFileName + _tcslen(theConfig.path_module);
			SetDlgItemText(IDC_EDIT_PATH_NANDRETRY, relative_path);
		}
		else
		{
			SetDlgItemText(IDC_EDIT_PATH_NANDRETRY, pstrFileName);
		}
	}
}

void CPageGeneral::OnCheckOtpSerial() 
{
	// TODO: Add your control notification handler code here
	BOOL check = FALSE;
	CString str;
...
void CPageGeneral::OnChangeEditOtpSerial() 
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CPropertyPage::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
	...
	if (str.GetLength() > 16)
	{
		AfxMessageBox(_T("lenght more than 16"), MB_OK);	
		str = str.Left(16);
		GetDlgItem(IDC_EDIT_OTP_SERIAL)->SetWindowText(str);
		GetDlgItem(IDC_EDIT_OTP_SERIAL)->SetFocus();
	}
